...
def swap_state_decoder_probe(
    ...
    """
    Correct workspace intervention.

    For each A/B pair, first obtain their normal carried workspace. Then keep
    the visible terminal state + goal fixed and substitute the OTHER member's
    workspace.

    We report:
      normal_correct:
          original workspace produces correct answer
      swapped_correct:
          swapped workspace produces the other member's answer
      swap_changes:
          workspace replacement changes the decision
      swap_directional:
          A workspace => A answer and B workspace => B answer
    """
    pairs=defaultdict(list)
    ...
    for pair in pairs.values():
        ...
        # Use A workspace on B and vice versa. The old V266 implementation
        # accidentally mixed up which result was being tested.
        a_swapped=step(
            model,
            b,
            b["horizon"]-1,
        ...
        a_swap=int(
            a_swapped["action_logits"].argmax().item()
        )
        b_swap=int(
            b_swapped["action_logits"].argmax().item()
        )

        ta=a["actions"][-1]
        tb=b["actions"][-1]

        normal_correct+=int(a_base==ta)
        normal_correct+=int(b_base==tb)

        swapped_correct+=int(a_swap==ta)
        swapped_correct+=int(b_swap==tb)

        swap_changes+=int(a_swap!=b_base)
        swap_changes+=int(b_swap!=a_base)
