...
def worker(args):
    ...
    print(
        ...
        f"freeze={metrics['freeze_accuracy']:.3f} "
        f"zero={metrics['zero_workspace_accuracy']:.3f} "
        f"drop_zero={metrics['normal_vs_zero_drop']:.3f} "
...
def main():
    ...
    # 0 = original task. >0 = auxiliary memory supervision diagnostic.
    p.add_argument(
        "--aux-memory-weight",
        type=float,
        default=0.0,
    ...
    p.add_argument(
        ...
        type=float,
        default=1.0,
        help="Weight for the diagnostic auxiliary-memory run.",
    )
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/v267"),
    )
    p.add_argument(
        "--dataset-output",
        type=Path,
        default=Path(
            "results/v267_memory_diagnostic_dataset.jsonl"
        ),
    )
    ...
    # Original model at 4 horizons, plus auxiliary-loss latent model at all 4.
    tasks=[]
    ...
    for h in (2,3,4):
        tasks.append(
            ("latent_workspace",h,args.aux_memory_weight_probe)
        )

    for h in (3,4):
        tasks.append(
            ("gated_workspace",h,args.aux_memory_weight_probe)
        )
    ...
    print(
        "=== V267 MEMORY FAULT-ISOLATION BENCHMARK ===",
        flush=True,
    )
    print(
        "horizons: 1 2 3 4",
    ...
    print(
        "architectures: baseline_graph latent_workspace",
        flush=True,
    )
    print(
        "auxiliary diagnostic weight:",
        args.aux_memory_weight_probe,
        flush=True,
    )
    print(
        "parallelism:",
        ...
        for proc,(arch,h,aux,output) in list(active.items()):
            ...
            if code!=0:
                for other in active:
                    if (
                        other is not proc
                        and other.poll() is None
                    ):
                        other.terminate()

                raise RuntimeError(
                    f"worker failed "
                    f"{arch} h={h} aux={aux} code={code}"
                )

            if not output.exists():
                raise RuntimeError(
                    f"worker missing result "
                    f"{arch} h={h} aux={aux}"
                )
    ...
    summary=args.output_dir/"v267_summary.json"
    summary.write_text(
        json.dumps(
            ordered,
            indent=2,
    ...
    print(
        "V267 MEMORY FAULT-ISOLATION SUMMARY",
        flush=True,
    )
    print(
        "="*100,
    ...
    for r in ordered:
        ...
        print(
            ...
            f"freeze={m['freeze_accuracy']:.3f} "
            f"zero={m['zero_workspace_accuracy']:.3f} "
            f"drop_zero={m['normal_vs_zero_drop']:.3f} "
            f"disc={m['pair_discrimination_rate']:.3f} "
            f"swap={s['workspace_swap_changes_decision_rate']:.3f} "
