...
| E4a: operators declare what they need and give | `Operator(needs=, gives=)`: an operator is proposed only once every slot it needs is in working memory, and `proposes` keeps only what it asks of their values (`relation == is_a`); one that goes on (CONTINUE) without writing what it gives raises `Unwired`. An executive built with `given=` -- the slots working memory starts with -- chains forward over the declarations (a subgoal gives what it returns) and refuses to be built if any operator could never be proposed: a need nothing gives is a wiring mistake, not a question declined. `_ask`'s fifteen operators and `where will` are declared, so the chain that was implicit in their conditions (resolve → referent → bind → rest → relation → walk → note) is now stated and checked on every turn. **Behaviour unchanged:** 1,056 tests; probe 114/114 as the baseline; bAbI test 98.1 / 0.1 / 1.8, and all 20,000 answers identical (prediction, outcome, reply, act) matched by task, story and line. This forward chaining is the search E5's means-ends step needs |
| E4b: the deciding path as operators | v684's `Engine.ask` is an executive (`v684`): read the question, nothing to ask about, senses, classify (R1), another sense and as a property (R27), between senses (R29), between words (R2 and the walk), corroborated (R19), describe, the answer. Only a step's *path* (a kind, a yes or no, a description) is its condition; what it finds in the answer is decided when it fires and said as CONTINUE (changed it) or DECLINED (left it), so a later step can never re-propose an earlier one, which the cascade never did. v687's `ask` is one executive (`v687`) over what was four stacked `ask` overrides: the pin that fights the question, the eleven layers, the norms (R17), a description (R16, `IdentifyingEngine.identified`), two subjects (R6, `BridgedEngine.bridge`), the fact graph (v684's run, kept beneath on `executed`), a folk category, the answer. The v684-v686 servers keep their own `ask`, now calling the same pieces. **Behaviour unchanged:** the whole payload of every audit question (2,258) identical before and after, rules legend included (two runs before agreed once `neighbourhood.edges`, built from a set, is sorted); 1,057 tests; probe 114/114; bAbI test all 20,000 answers identical; audit rows identical to the E3 run. **Audit credit now names what decides:** the norms fired on 990 questions, mean +0.81; classify 340, +0.22; another sense 23, +0.22; a folk category 5, +1.0; between words 897, -0.01 (856 of them a screened or corrupted negative left unknown, worth 0); corroborated changed 47. **The six reversals the ledger reports are not choices learning should make:** `refused by name` (answered 2, mean 0) against `the norms` (+0.81) compares means over *different questions* -- a layer declines what is not its kind, so an operator's mean is the base rate of the questions it takes, not what it would have done on the ones it was passed over for. Promoting the norms above the refusal would have the norms answer what R18 refuses. A utility learned from these means would reorder specialists by their populations. What a reversal needs is the counterfactual: on the same question, what the passed-over operator would have answered |
| E4, counterfactual credit | `executive.suppressed` asks the same question again as though named operators were not there; `v688/counterfactual.py` does it for every audit question -- each operator that did something suppressed in turn, further while something still answers (12 runs, 3 deep at most) -- and scores each against the gold: **marginal** (reward with it less without it, on the same question), **competition** (who decides instead, and both rewards), and an **oracle** (the best any explored suppression reached). 2,232 questions, 18,495 runs, 5.4 minutes. **The order written is right, and there is almost nothing for utilities to learn.** As shipped, mean reward +0.387; the best reachable by choosing, per question, among every suppression explored is +0.411 -- an upper bound no ordering can reach. The only real competition is the norms against the fact graph, on 726 questions: +0.928 against -0.003, the norms first as written (700 helped, 27 hurt). Every other competition is one question. R19 (`corroborated`) is worth +0.87 where it acts (44 helped, 3 hurt). What is left is in operators' **conditions**, not their order: `another sense` helped 8 and hurt 10 (a crab read as another sense is an insect, a kite is a living thing -- the existential reading of the subject, and several are fair readings the screened gold calls false); `as a property` hurt all 9 it changed, every one a false claim R27's exclusion had denied for the wrong reason (`is a whale underwear`), since this gold holds none of the false denials of true properties it was written for (`is a television modern`); `refused by name` cost one (`can a bench seat more than one person`, which the norms would have verified). Those are conditional on the question, which a scalar utility cannot express, and forty questions are too few to fit what could. **Learning stays off.** If it is ever turned on it should be a predictor of an operator's outcome from the question (the reader's encoder, a small head: JEPA's predict-before-acting, over reasoning steps rather than world states), trained on counterfactual runs like these -- after E5, and only with far more disagreeing questions than this gold has |
| E4c: effects | An operator declares the stores outside working memory it may change (`Operator(effects=)`): `conversation` (this conversation's individuals and events), `knowledge` (kinds taught in it), `definitions` (glosses read into definitions memory). The stores say when they are changed, where they are written -- `EpisodicMemory._record`, the one command every event goes through, and `DefinitionMemory.keep_all` -- by calling `executive.effect(store, what, undo)`, which keeps the change on the step of the operator running and raises `Unwired` unless **every** operator running declared the store (the act, and the question it asked inside it): a planner reads declarations, so one that changed what it did not name would make the plan wrong. A subgoal that returns nothing takes its effects back, the last first, so E1's promise that a failed subgoal leaves nothing behind holds outside working memory too; one that returns keeps them as the goal's that pushed it. Definitions memory can take a gloss back (`forget`, only for one not there before); episodic events are appended and replayed, so they carry no undo -- a plan that may fail must not tell before it knows. Declarations came from a survey (undeclared effects logged over the suite and probe: 656 `tell`, 166 `introduce`, 60 `teach` ...) and from which acts reach v688, which reads glosses; then made strict. **Behaviour unchanged:** 1,073 tests; probe 114/114; bAbI test all 20,000 answers identical, and no undeclared effect in any of them. Retract, pin and consult as operators of their own, and an impasse resolved by one, are E5's: nothing today is a subgoal with effects |
| E5: means-ends | An executive that `plans` needs no impasse named and no subgoal wired to one. Stuck, it takes the most useful waiting operator that lacks only slots something could give, and pushes a subgoal of the operators that give any of them -- its own, or its library of `means`, operators never proposed on their own -- run `until` the slots are in that goal's frame. That subgoal plans in turn for what *its* operators need; a slot already being pursued by a goal beneath is never pursued again (`_PURSUING`), so a circle of needs ends; what fired on the way is not proposed again; and what could not be achieved is written to `unachieved`, for an operator that answers when nothing could be found. The subgoal's effects are kept or taken back as E4c's are. `Executive.plan(wanted, given)` is the same search done without running anything -- regression over needs and gives, the most useful way to each slot first -- the order the declarations allow, not the order written. Planning is opt-in: every cascade converted before it is ordered as written and stops at its impasse, so nothing it answered can move. `where will` is the first executive to plan: `kept there` is its means to `found`, no longer a subgoal wired to the impasse `no place here is for it`. **Behaviour unchanged:** 1,080 tests (7 new: achieved in turn, unachieved said, a circle ends, a failed means undoes its effects, the static plan, reachability through means); probe 114/114; bAbI test all 20,000 answers identical, 322 of them planned |
| consult, measured and not built | E4c left `consult` -- an operator whose effect is reading a WordNet gloss into definitions memory -- for a planner to reach when a question is stuck. Measured first, on the audit's own questions (screened gold, 250 per rung, 300 corrupted): of 2,258 questions the shipped engine leaves **1,139 unknown**, and matching each subject's definition facts (`state/v689-definitions.sqlite`, read by `GlossReader`) against the question's target the way v687 matches a fact settles **4** of them -- 2 right, 2 wrong; requiring the fact's relation to be the one asked leaves **1**, and it is wrong (`is yoghurt produced by mammary glands`, against `yogurt.n.01 has_property custard-like`); requiring R28's plain match leaves none. A one-line gloss says what a thing *is*, and this gold asks what it *has* and *can do*, which is the same reason the norms answer these and the crawl does not. **So the operator is not built**: there is nothing here for it to reach. Definitions already sit in v689's own walk (`EpisodicReasoner.facts_of`), where a conversation asks what a word means |
| E6: chunking | What a means-ends subgoal came to, kept as a rule (`Chunks`, Soar's chunking): the impasse's shape -- the executive, the slots wanted, and the slots the goal already had -- against the operators that did something, in the order they fired. The next impasse of that shape offers exactly those, in one executive, instead of searching for them again; they still have to propose, and do what they always did, so a chunk can save the search and never answer in its own right. One that does not come to the slots is forgotten and the search follows it in the same cycle. Procedural memory is about the executive's own operators, not about any conversation, so one `Chunks` is shared by every session in the process. **A chunk is kept only where the subgoal pushed subgoals of its own,** and that rule was measured, not assumed: keeping every one cost **20 extra subgoal pushes for 21 chunks** on the bAbI test split, because at one step deep whether the step answers depends on the question, not on the shape of the impasse, so the chunk was tried, declined and followed by the search anyway. With the rule, bAbI does exactly what E5 did -- 322 planned answers, 322 pushes, 1,015 operators fired, no chunks -- and the saving is left for searches deeper than the one place that plans today. **Behaviour unchanged:** 1,084 tests (4 new: the second time is not searched for, what a chunk keeps, a chunk that stops working is forgotten, an executive with no chunks keeps none); probe 114/114; bAbI test all 20,000 answers identical |
| V1: the trace crosses the boundary | v689 asks v688 over a socket, and `Answer.as_dict` carried the verdict, steps, evidence and parse but not `executed`: what the executive ran inside a served answer did not exist for whoever asked it. It travels now, on the answer and on the run, and a v689 turn keeps those runs beneath its own, each with the question it was put (`Session._kept_beneath`). Credit and the counterfactual read served answers, not only direct engine calls |
| V2: v688's loop as an executive | The loop was the last cascade: `attend -> rephrase -> seed -> for eight cycles (ask the pool, teach, record, queue) -> challenge -> summarise`, with the cycle a `for` and a `break`. It is nine operators now, and what repeats is the conditions -- something open to ask, and cycles left to ask it in (`Operator(repeats=True)`, new: an operator that may fire again once its conditions hold again). The teacher is declared as an effect, the only one here that reaches outside. **Behaviour unchanged:** 40 probe utterances replayed through the loop before and after, every run identical to the field except clocks; 1,090 tests; probe 114/114; bAbI test all 20,000 answers identical. **What it cost, and what came of it:** the teacher operator repeated on the condition `there are answers`, which its own action left standing, so it fired for ever and starved every operator under it -- a hung suite, a process at 42 GB, and two hours before it was noticed. The condition is now `this cycle has not been reviewed`; `Executive.LIMIT` (10,000 firings) raises `Runaway` naming the last operator, so the same mistake fails in a second; and `Working.pop` is `Working.leave`, because a stack method under the dict's own name meant `memory.pop("pending")` closed the goal instead of taking the slot. **What the loop actually does, measured on those 40:** 2.85 cycles per utterance (1 to 6), 7 questions asked, and **0 of 40 runs settle** -- it stops because it runs out of questions or cycles, never because nothing is left open |
| V3: what to ask next, as operators | `Generator.queue` was a list of calls and a sort -- its own docstring called it "the whole of executive control at this stage: a sort, not a subsystem". It is eleven operators now (carried, a gap to close, a doubt to put, which sense was meant, the same asked of another sense, what an answer requires, a claim in two halves, what an answer said, the best of them, curiosity fills the room left, what to ask), the ranking unchanged and the order written kept as their utilities, and each cycle's run travels on `Run.executed` beside the loop's own, so a source can be credited for what it asked and suppressed one at a time. **Behaviour unchanged:** the 40-utterance loop replay identical field by field; 276 v688 tests. **A condition must not consume what it tests:** `buffer.open_gaps`, `open_doubts` and `open_senses` hand their pending list over and clear it, so a `proposes` that asked whether there was anything spent it before the action ran, and every carried family check was lost (15 of 40 runs shallower: `is an ant tiny` fell from 6 cycles to 3). Those sources propose always and decline when they find nothing, and `Operator.proposes` now says so. **What the sources actually do, over 40 probe utterances and 114 cycles:** curiosity fills the room 99 times, a doubt 16, carried 12, a gap 3, a split 2, a sense 1 -- and `what an answer requires`, `what an answer said` and `the same asked of another sense` **never fired at all**. The loop is not shallow because it runs out of cycles; it is shallow because what comes back rarely opens anything, and curiosity, ranked last, is what fills the width |
| V4: why the loop is shallow -- the material, not the executive | `v688/depth.py` runs the loop over a set of utterances and traces every call of every question source to the line it returned from. **Three sources never fired on the probe, and two by design:** `the same asked of another sense` only on a *seed that was denied* (280 of 282 calls stopped there), `what an answer said` only on a *definition that was asked for*. The third is the graph: `what an answer requires` reads what an action needs, derived by cue validity over the store (`graph.Requirements`), and over its **300 commonest one-word abilities a requirement derives for 38, most of them wrong** (`benefit -> website`, `hear -> tooth`, `respond -> website`); the sound ones are about six (`run`, `stand` -> leg, `fly` -> wing, `climb` -> claw, `operate` -> engine). The crawl records what things have, not what actions need. **Asked about what the store is dense on, the same loop goes deeper:** 36 questions, the ten animals with the most facts x the four actions it derives correctly -- **4.69 cycles and 14.5 questions per utterance against the probe's 2.85 and 7.0**; the requirement source gives 22 times (0 on the probe), doubts 42 (19), gaps 13 (3). Adrian's point, measured: the benchmarks are one-hop questions about sparsely connected concepts, and read as a limit of the architecture they were a limit of the material. **`settled` could never be true:** it required `recent` empty, and `recent` is the last cycle's answers, set by every `record` and never cleared -- 0 of 76 runs settled, when every one of them had run out of questions (none was cut off at the cycle limit). It now means what its docstring says. 277 v688 tests |
| V5: what an action needs, from the norms | The crawl records what things have, not what actions need, so `graph.Requirements` derived a part for 38 of the store's 300 commonest abilities, most wrong. The rated norms rate `can fly` and `has wings` for the same concepts -- but `load_xcslb` read COMPS' pair file, which samples at most ten holders a property (`can fly`: 10, not 54), and the full matrix ships without column labels. **`corpora.xcslb_holders` places the columns:** COMPS drew each property's foils from its column's zeros, so the column is the one holding every holder and none of the foils; 1,277 place at once, 1,493 after six rounds of elimination, **no column claimed twice**, 22,477 of the 30,009 ratings back, and where candidates remain a concept counts only if every candidate gives it the property. **`graph.NormRequirements`** ranks a part by how many kinds of doer share it, how much commoner it is among doers than among non-doers of their kind (a part measured at no difference -- `swim -> mouth` -- is dropped), share, and precision in tenths, and breaks a tie toward the part the store records most, since only a part the store knows can come back denied (`fish has_part "no legs"`; forelimbs of fish are not recorded). **On a hand list of 25 actions and what they need: 15 right and 24 answered, where the crawl's derivation got 3 and answered 6.** Rules tried and worse: co-occurrence alone 13, without precision 14, WordNet body parts only 11, the store's commonest part as tie-break 12. Decisive -- enough to say a denial is not about anatomy -- needs two kinds, +0.25 contrast and 0.7 precision: `fly -> wing`, `walk -> foot`, `cut -> edge`, `hunt -> claw`, all right (`bite -> foot` was decisive before the precision clause). The crawl answers where the norms do not rate an action, as before. **It did not make the loop deeper**: probe unchanged (its questions are properties, not actions, and the source is stopped at the question type), densest kinds 4.56 cycles against 4.69 -- the follow-ups are *right* now (`does a fish have legs`), and a few come back already settled (`sparrow` / `wing`). Depth counts cannot see that, so the proving ground has to score answers. `does a beagle swim` now asks `does a beagle have legs`, a real question: the store records legs of horses and not of dogs. 1,094 tests; probe 114/114; bAbI test all 20,000 answers identical |
| V6: the loop searches but cannot conclude | `depth.py --proving`: 100 questions `can a <concept> <verb>` over the 25 abilities with the most screened foils -- positives from XCSLB's recovered matrix, negatives from COMPS foils a judge screened as false (XCSLB's zeros are silences: a category-exclusive reading of them said an alligator cannot hurt and a budgie cannot climb). Scored twice, the engine's answer to the question as asked and the loop's after its cycles: **31 right, 4 wrong, 65 unknown both times; the loop ran 3.35 cycles, asked 9.3 questions each, and changed no answer.** That is the design: a run's outcome is `confidence.of_run` over the seed's answer, and the loop can knock a yes down (`overturned`), re-read it under another sense, or have a teacher ratify it -- nothing it finds can turn an unknown into a yes or a no. What its follow-ups found on the 65, scored as conclusions: **the subject lacks what the action needs -> no** fires on 4 and is right on 4 (`does a train have hulls`: no, so a train does not sail) -- modus tollens, sound; **it has what the action needs -> yes** fires on 16 and is right on 9 (a porcupine has legs and does not run, an ox has muscles and does not gallop) -- affirming the consequent, a coin flip, and not to be built. And the step a planner would take is never taken: `can an ant sleep` confirmed an ant is an insect and never asked whether insects sleep |
...
| step | what it is |
| --- | --- |
| V11.0: is the gold right? **Yes -- done** | Fixing the engine to match wrong gold is the trap `data/xcslb.SOURCE.md` documents, and it would have been the second time, so the 22 were screened again before a line of engine code moved. Three looked like judge errors: an artichoke *is* planted, geranium petals *are* edible, turtles *do* have inner ears. The screen that labelled them ran one prompt, `CAREFUL`, which opens "Most claims put to you are false"; `CREDULOUS` is its mirror and `CAPABLE` its possibility-for-an-ordinary-member variant. **All 18 screened denials survive both opposite leans -- none flips.** The suspicion was not baseless: those three are exactly where the second opinion is softest (`credulous` 0.729 for the artichoke, 0.703 for the turtle, **0.553** for the geranium, against `careful` at 0.96 or better), so they should not be the last three chased -- but soft is not overturned, and the labels stand. **Controlled**, because 18 of 18 surviving means nothing if the opposite lean denies everything: on 40 claims XCSLB states, `credulous` says yes to 88% and `capable` to 85%, against `careful`'s 82%. The instrument works and the result is real. This also re-tests `teacher.py` §26's claim that a credulous no agrees with every confident careful no, which had been measured over the whole corpus and not on its hardest cases; it holds. The other 4 of the 22 are not foils at all -- they are the genuine positives the matrix gains (`stick insect has a head`, `can lay eggs`, `dressing gown is made of soft fabric`, `sweet potato has a green stalk`). **So the target is all 22, unrevised.** Scratch: `scratchpad/v11/screen22.py`, `control.py` |
