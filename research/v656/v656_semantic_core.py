...
class Graph:
    def __init__(self, db, cache_entries=12000):
        ...
        # The semantic facts remain immutable by convention. V656 writes only
    ...
    def relation_frame_lookup(
        ...
        row = self.conn.execute(
            ...
            WHERE decision_type='relation_from_frame_v656'
              AND context_key=?
    ...
    def relation_frame_learn(
        ...
        self.conn.execute(
            """
            DELETE FROM distilled_decisions
            WHERE decision_type='relation_from_frame_v656'
              AND context_key=?
            """,
            (frame_key,),
