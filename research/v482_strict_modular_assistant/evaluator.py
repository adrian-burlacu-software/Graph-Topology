...
class CandidateEvaluator:
    ...
    No candidate is authoritative because it came from the LLM or graph.
    Goal fit and context must dominate source prestige.
    """
    ...
    def _goal_score(self,goal,content):
        ...
        if goal.name=="request_action":
            return 7.0 if n>=3 else 2.0
    ...
    def _evidence_score(
        ...
        if source=="knowledge":
            return 7.0 if has_knowledge else -5.0
    ...
    def _progress_score(self,goal,content):
        ...
        if goal.name=="request_information":
            return 2.0 if len(words(content))>=3 else 0.0

        return 1.0

    def _naturalness(self,content):
        low=content.lower()
        if any(x in low for x in META_PHRASES):
            return -4.0
