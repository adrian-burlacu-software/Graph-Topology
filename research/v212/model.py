...
class TeacherDistilledController(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Goal mode describes the *state-level objective*, not the action label.
        # This lets deterministic supervision distinguish e.g. "create a node"
        # from "branch from this node" without leaking ACTIONS into the input.
        ...
        # Explicit relevance/attention supervision. This is intentionally
        # separate from source/target pointer heads: the controller first
        # learns what part of working memory matters, then what to do with it.
    ...
    def encode_goal(
        ...
        device: torch.device,
        goal_mode: int = 0,
        goal_target_concept: str | None = None,
    ) -> torch.Tensor:
        if (
            0 <= goal_source_index
            < len(state.nodes)
        ...
        if (
            ...
            target = self.concept_embedding(
                state.nodes[goal_target_index].concept,
                device,
            )
        elif goal_target_concept:
            target = self.concept_embedding(goal_target_concept, device)
        else:
            target = torch.zeros((1, self.hidden_size), device=device)
        ...
        mode = self.goal_role_embedding(
            torch.tensor(
                [max(0, min(goal_mode, self.goal_role_embedding.num_embeddings - 1))],
                dtype=torch.long,
                device=device,
            )
        )
    ...
    def predict_attention(
        ...
        """Return one relevance logit per working-memory node."""
        goal = goal_state.expand(node_state.shape[0], -1)
        return self.attention_head(
            torch.cat(
                [node_state, goal],
                dim=-1,
            )
    ...
    def attended_graph(
        ...
        """Differentiable attention pooling used by the downstream controller."""
        weights = torch.sigmoid(attention_logits)
        weights = weights / weights.sum().clamp_min(1e-6)
        return (node_state * weights.unsqueeze(-1)).sum(dim=0, keepdim=True)
