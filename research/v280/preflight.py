...
def main():
    ...
    attended_pos=source.find("        attended = (")
    memory_pos=source.find("        if slow_memory_state is None:")
    assert attended_pos >= 0
    assert memory_pos > attended_pos, (
        "persistent-memory update occurs before attended is computed"
    )
    print("persistent-memory ordering: PASS",flush=True)
    ...
    print(
        "=== V280 LONG-HORIZON MEMORY PREFLIGHT ===",
        flush=True,
    )
    print(
    ...
    print(
        "V280 LONG-HORIZON MEMORY PREFLIGHT: PASS",
        flush=True,
    )
