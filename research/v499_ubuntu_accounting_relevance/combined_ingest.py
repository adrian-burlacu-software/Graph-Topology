...
def main():
    ...
    totals["ubuntu_pairs"]+=u//2
    totals["ubuntu_decoded_turns"]+=u
    ...
    ubuntu_pair_count=con.execute(
        "SELECT COUNT(*) FROM ubuntu_pairs"
    ).fetchone()[0]
    ubuntu_token_count=con.execute(
        "SELECT COUNT(*) FROM ubuntu_pair_tokens"
    ).fetchone()[0]
    totals["ubuntu_pairs"]=ubuntu_pair_count
    totals["ubuntu_token_rows"]=ubuntu_token_count
    totals["ubuntu_decoded_turns"]=(ubuntu_pair_count*2)
