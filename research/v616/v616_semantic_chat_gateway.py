...
class Graph:
    ...
    def resolve_entity(
        ...
        """
        Resolve a natural-language entity mention using only the graph DB.

        Strategy:
          1. exact subject match (case-insensitive);
          2. exact object match;
          3. bounded LIKE search over the indexed subject/object values.

        Returns ranked candidate canonical node strings.
        """
        if mention is None:
            return []
        ...
        # Prefer exact subject matches because search starts from subjects.
        ...
        # Exact object matches can still identify a node that also appears as
        # a subject elsewhere.
        for row in self.conn.execute(
            """
        ...
        # Fallback is intentionally bounded. It is not a full-table scan.
        pattern = f"%{mention}%"
        ...
        for row in self.conn.execute(
            ...
            if value is not None and value not in seen:
                ...
                score = (
                    0.90
                    if self._norm_entity(value) == norm
                    else 0.60
                )
                candidates.append(
                    {
        ...
        for row in self.conn.execute(
            ...
            SELECT DISTINCT object
            FROM edges
            WHERE object LIKE ? COLLATE NOCASE
            LIMIT ?
            """,
            (pattern, limit),
        ).fetchall():
    ...
    def subject(
        ...
        if parse.entities:
            ...
            if graph is not None:
                resolved = graph.resolve_entity(
                    mention,
                    limit=16,
                )
                if resolved:
                    best = resolved[0]
                    self.active_subject = best["node"]

                    record = self.entities[
                        mention.lower()
                    ]
                    record["canonical"] = (
                        best["node"]
...
def hypotheses(
    ...
    if graph is not None and mention:
        try:
            resolution_candidates = graph.resolve_entity(
                mention,
                limit=16,
            )
        except Exception:
...
def trace_result(text, parse, hs, selected, search_result, ctx):
    # V616: keep the trace serializer on the current context-aware API.
    return {
        "timestamp": time.time(),
        "text": text,
        "parse": asdict(parse),
...
def _learn_from_resolution(
    ...
    # Train the language/context -> first relation attention.
    ...
    # Train the existing goal-conditioned graph path prior.
    prefix = ()
...
def smoke(
    ...
    # Pick a real graph subject with at least one target relation. This makes
    # the smoke test exercise entity resolution + direct proof rather than
    # depending on an arbitrary external example entity.
    smoke_subject = None
    smoke_relation = None
    ...
    try:
        for relation in vocab:
            ...
            if row and row[0] is not None:
                smoke_subject = row[0]
                smoke_relation = relation
                break
    except Exception:
        pass

    if smoke_subject is None:
        raise RuntimeError(
            "Smoke could not find a graph subject with a target relation"
        )

    inputs = [
        "hello",
        f"Tell me about {smoke_subject}.",
        f"What is the {smoke_relation} of {smoke_subject}?",
        "What can you tell me about it?",
    ]

    traces = []

    print("=== V616 SMOKE ===", flush=True)
    print(
        f"graph-grounded subject : {smoke_subject}",
        flush=True,
    )
    print(
        f"graph-grounded relation: {smoke_relation}",
        flush=True,
    )
...
def main():
    ...
    print("=== V616 SEMANTIC CHAT GATEWAY ===", flush=True)
    ...
    try:
        ...
        if args.mode == "smoke":
            ...
            ctx = memory
            print("=== V616 CHAT (type 'exit') ===", flush=True)
        ...
        print()
        print("=== V616 COMPLETE ===", flush=True)
