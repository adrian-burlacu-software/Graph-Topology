...
def main():
    ...
    if args.teacher_probe:
        ...
        for _ in range(args.teacher_probe):
            t=next_curiosity(
                con,
                exclude_keys=probe_exclusions,
            )
            if not t:
                break
        ...
        print(
            f"      curiosity target scan "
            f"time={time.perf_counter()-probe_started:.2f}s "
            f"targets={len(probe_targets)}",
            flush=True,
        )
    ...
    q_path=RESULTS/"v453_curiosity_questions.jsonl"
    f_path=RESULTS/"v453_curiosity_failures.jsonl"
    report_path=RESULTS/"v453_assistant_curiosity_report.json"
