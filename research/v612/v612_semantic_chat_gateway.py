...
def trace_result(text, parse, hs, selected, search_result, ctx):
    # V612: keep the trace serializer on the current context-aware API.
    return {
        "timestamp": time.time(),
        "text": text,
        "parse": asdict(parse),
...
def _learn_from_resolution(
    ...
    """
    Consolidate a resolved hypothesis into persistent semantic memory.

    Context intentionally exposes `record_turn()` and `remember_path()` rather
    than a separate single-relation learner. Keep this adapter aligned with
    that contract.
    """
    if not selected:
        return
    ...
    # Record relation-level outcome without relying on a nonexistent method.
