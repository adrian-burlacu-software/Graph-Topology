...
ARCHITECTURES={
    ...
    # 1. Combine the two strongest mechanisms.
    "eligibility_transition":dict(
        state_mode="gated_latent",
        attention_workspace=False,
        explicit_progress=True,
        direct_goal_to_workspace=False,
    ...
    # 2. Adaptive eligibility decay rather than fixed exponential decay.
    "adaptive_eligibility":dict(
        state_mode="gated_latent",
        attention_workspace=False,
        explicit_progress=True,
        direct_goal_to_workspace=False,
        ...
        terminal_query=False,
        action_memory_binding=False,
        training_mode="adaptive_eligibility",
    ),
}
...
def adaptive_eligibility_loss(
    model,
    item,
    device,
):
    """
    Learn an explicit timestep-dependent credit weight while constraining it
    to decrease smoothly toward the terminal step. The weight is generated
    from the step's progress embedding, so the model can preserve credit when
    a longer horizon actually needs it.
    """
    if item["horizon"]<=1:
        return torch.zeros((),device=device)
    ...
    for t in range(1,horizon):
        ...
        # Deterministic weighting keeps this objective stable and removes the
        # need for another learned module just to choose credit magnitudes.
        weight=0.5+0.5*base
...
def combined_credit_loss(
    model,
    item,
    device,
):
    """
    Combine the empirically strongest mechanisms without introducing another
    storage mechanism:
      - eligibility credit to earlier states
      - transition-aware supervision on the state update
    """
    return (
        eligibility_loss(
            model,item,device
...
def adaptive_credit_loss(
    model,
    item,
    device,
):
    """
    Adaptive eligibility plus transition supervision.
    """
    return (
...
def training_objective(
    ...
    if training_mode=="adaptive_eligibility":
        return (
            terminal
