...
def main():
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/v284"),
    )
    p.add_argument(
        "--dataset-output",
        type=Path,
        default=Path("results/v284_memory_read_dataset.jsonl"),
    )
    args=p.parse_args()
    ...
    print(
        "=== V284 MEMORY READ-PATH BENCHMARK ===",
        flush=True,
    )
    print(
        "architectures:",
    ...
    summary=args.output_dir/"v284_summary.json"
    summary.write_text(
