# V676 — JEPA Semantic State

V676 inherits the preceding laboratory and contextual-memory baselines. It is
the additive experiment baseline for predictive semantic-state and next-state
learning while preserving graph-grounded chat behavior.

V676 mirrors V672 and turns its specialist evidence into a bounded semantic
...
V676 assigns slots across the entire interval:
...
V676 keeps per-worker evidence tables and materializes an aggregate view. Imported shared state is not re-exported as new worker evidence.
...
The V671 composition lane produced ~55k learned records in the observed run. V676 defaults to:
...
`v676_inspect.py` additionally shows:
...
Use a fresh V676 shared database for this experiment. Do not reuse the V671 shared database because V676 has a new evidence schema.
...
```powershell
python .\research\v676\v676_semantic_network_builder.py --conceptnet ".\data\conceptnet-assertions-5.7.0.csv.gz" --output ".\data\v676_focused_semantic.sqlite" --focus-concepts bear dog animal --focus-depth 2 --progress-every 250000
```

```powershell
python .\research\v676\v676_runtime.py --database ".\data\v676_focused_semantic.sqlite" --output ".\results\v676_chat.json" --trace-output ".\results\v676_chat_traces.jsonl" --memory-output ".\results\v676_memory.json" --worker-log-dir ".\results\v676_workers" --shared-memory ".\results\v676_shared_memory.sqlite" --spacy-model en_core_web_sm --llm-model "C:\Users\adria\Desktop\dev\Graph-Topology\llm\SmolLM3-3B" --mode chat --max-hypotheses 12 --goal-budget 40 --per-node 60 --max-depth 3 --cache-entries 12000 --checkpoint-seconds 300 --seed 67600 --batch-sleep 0.20
```

For a faster checkpoint-spacing test:

```powershell
python .\research\v676\v676_runtime.py --database ".\data\v676_focused_semantic.sqlite" --output ".\results\v676_chat.json" --trace-output ".\results\v676_chat_traces.jsonl" --memory-output ".\results\v676_memory.json" --worker-log-dir ".\results\v676_workers" --shared-memory ".\results\v676_shared_memory_1min.sqlite" --spacy-model en_core_web_sm --llm-model "C:\Users\adria\Desktop\dev\Graph-Topology\llm\SmolLM3-3B" --mode chat --max-hypotheses 12 --goal-budget 40 --per-node 60 --max-depth 3 --cache-entries 12000 --checkpoint-seconds 60 --seed 67600 --batch-sleep 0.20
```

Inspect after a run:

```powershell
python .\research\v676\v676_inspect.py --shared-memory ".\results\v676_shared_memory.sqlite" --events 40
```
