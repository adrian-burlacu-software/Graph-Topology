...
from v676_offline_learning import worker_main
from v676_memory import SharedCheckpoint


def build_parser():
    ap = argparse.ArgumentParser(description="V676 19 offline workers + 1 online semantic chat worker")
    ap.add_argument("--database", required=True)
    ap.add_argument("--output", default="./results/v676_chat.json")
    ap.add_argument("--trace-output", default="./results/v676_chat_traces.jsonl")
    ap.add_argument("--memory-output", default="./results/v676_memory.json")
    ap.add_argument("--worker-log-dir", default="./results/v676_workers")
    ap.add_argument("--shared-memory", default="./results/v676_shared_memory.sqlite")
...
def launch_offline(args, stop_event):
    ...
    for worker_id in range(19):
        p = mp.Process(target=offline_process_entry, args=(child_args, worker_id, stop_event), name=f"v676-offline-{worker_id:02d}")
        p.daemon = True
        p.start()
        processes.append(p)
    return processes
...
def offline_process_entry(args, worker_id, stop_event):
    try:
        ...
        print(f"[V676 offline worker {worker_id}] FATAL: {exc!r}", flush=True)


def run_chat(args, stop_event):
    from v676_semantic_chat_gateway import run_chat_worker
...
def main():
    ...
    print("=== V676 COGNITIVE RUNTIME ===", flush=True)
    ...
    try:
        ...
        for p in offline:
            p.join(timeout=1.0)
        print("=== V676 RUNTIME COMPLETE ===", flush=True)
