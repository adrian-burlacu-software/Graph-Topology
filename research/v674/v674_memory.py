...
class RamSemanticMemory:
    ...
    V674 distinguishes local evidence from imported shared state. Imported rows
...
class SharedDistilledMemory:
    def __init__(self,graph,ram,checkpoint=None):
        self.graph_memory=__import__("v674_semantic_core",fromlist=["DistilledMemory"]).DistilledMemory(graph); self.ram=ram; self.checkpoint=checkpoint
    def lookup(self,decision_type,surface,context_text,candidates):
