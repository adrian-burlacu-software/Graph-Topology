...
def episode_loss(
    ...
    # The target graph can contain additional nodes after CREATE/BRANCH.
    # Transition prediction is defined over the nodes that existed in the
    # current working-memory state, so align target activations by concept
    # rather than assuming equal node counts.
    target_by_concept = {
    ...
    target_activations = torch.tensor(
        [
            target_by_concept.get(
                node.concept,
                0.0,
            )
            for node in example.state.nodes
        ],
        dtype=torch.float32,
        device=device,
    )

    transition_prediction = torch.sigmoid(
        transition_logits
    )

    if transition_prediction.shape != target_activations.shape:
        raise RuntimeError(
            "Transition target alignment failed: "
            f"pred={tuple(transition_prediction.shape)} "
            f"target={tuple(target_activations.shape)}"
        )
