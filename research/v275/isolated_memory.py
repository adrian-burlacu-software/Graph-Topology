...
def main():
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/v275"),
    )
    p.add_argument(
        "--dataset-output",
        type=Path,
        default=Path("results/v275_memory_read_dataset.jsonl"),
    )
    args=p.parse_args()
    ...
    print(
        "=== V275 MEMORY READ-PATH BENCHMARK ===",
        flush=True,
    )
    print(
        "architectures:",
    ...
    summary=args.output_dir/"v275_summary.json"
    summary.write_text(
