...
from v671_memory import RamSemanticMemory, SharedCheckpoint
    ...
    if lane in RELATION_GROUPS and RELATION_GROUPS[lane]:
        ...
        for subject in subjects:
            ...
            for row in rows:
                ...
                if back:
                    ram.upsert_knowledge(lane, subject, rel, obj, {"inverse_same_relation": True}, positive=1, confidence=0.9, source="offline_graph_analysis")
                inspected += 1
        ...
        for subject in subjects:
            ...
            for row in rows:
                ...
                back = conn.execute("SELECT 1 FROM edges WHERE subject=? AND relation=? AND object=? LIMIT 1", (obj, rel, subject)).fetchone()
                if back:
                    ram.upsert_knowledge(lane, subject, rel, obj, {"symmetric": True}, positive=1, confidence=0.8, source="offline_graph_analysis")
                inspected += 1
            ...
            for pos in first:
                ...
                for neg in rows[8:20]:
                    ...
                    if neg_rel == pos_rel or neg_obj == pos_obj:
                        continue
                    ram.upsert_knowledge(lane, subject, pos_rel, neg_obj, {"positive_relation": pos_rel, "distractor_relation": neg_rel, "positive_object": pos_obj}, negative=1, confidence=0.5, source="offline_counterrelation")
                    learned += 1
                    inspected += 1
                    break
        ...
        for subject in subjects:
            rows = fetch_outgoing(conn, subject, 48)
            for row in rows[:12]:
                mid, rel1 = str(row[2]), str(row[1])
                for row2 in fetch_outgoing(conn, mid, 24)[:12]:
                    rel2, obj = str(row2[1]), str(row2[2])
                    ram.upsert_transition(rel1, rel2, confidence=0.5, count=1)
                    ram.upsert_knowledge(lane, subject, rel1 + "->" + rel2, obj, {"middle": mid}, positive=1, confidence=0.4, source="offline_composition")
                    learned += 1
                    inspected += 1

    elif lane == "goal_relation_statistics":
        # Read semantic-goal decisions already shared online/offline and derive
        # empirical goal->relation associations without calling the LLM.
        ram_rows = ram.conn.execute("SELECT decision_type,selected,count,confidence FROM semantic_decisions ORDER BY updated_unix DESC LIMIT 5000").fetchall()
        for row in ram_rows:
            if str(row[0]) != "semantic_goal":
                continue
            goal = str(row[1])
            ram.upsert_knowledge(lane, goal, "goal_usage", None, {"count": int(row[2]), "confidence": float(row[3])}, positive=int(row[2]), confidence=float(row[3]), source="offline_goal_analysis")
            inspected += 1
    ...
    try:
        while True:
            ...
            try:
                ...
                log("analysis_batch", batch=batches, lane=lane, inspected=inspected, learned=learned, duration_s=elapsed, ram=ram.counts())
            ...
            # Current-time modulus provides staggered write slots. We still check
            # frequently, but only the worker owning the current slot writes.
            if shared.should_sync():
                try:
                    shared.heartbeat(role, os.getpid(), batches=batches, items=items, learned=learned_total, imported=imported_total, errors=errors)
