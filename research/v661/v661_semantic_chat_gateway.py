...
from v661_semantic_core import (
    Graph,
    DistilledMemory,
    candidate_senses,
    Context,
...
class LiveSemanticTeacher:
    ...
    def choose(self, question, candidates, decision_type, descriptions=None, frame=None):
        ...
        if decision_type == "relation_from_frame_v661":
            prompt=(
...
def apply_live_distillation(
    ...
    # Contextual follow-up: the semantic relation is authoritative. The graph
    # searches only that relation; no fact-level teacher may override it.
    if (
        selected0.subject
