...
class CognitiveBridge:
    ...
    def _memory_candidate(self, subject: str, kind: str) -> str | None:
        """Read semantic evidence conservatively from the preserved SQLite ingestion DB."""
        if self.memory is None:
            return None
        try:
            tables = [
                row[0]
                for row in self.memory.execute(
            ...
            for table in ordered:
                ...
                s_col = lower.get("start") or lower.get("subject") or lower.get("subject_id")
                ...
                if not (s_col and r_col and e_col):
                    continue
                if s_col.endswith("_id"):
                    continue
                rows = self.memory.execute(
                    f"SELECT [{r_col}], [{e_col}] FROM [{table}] WHERE lower(CAST([{s_col}] AS TEXT))=? LIMIT 12",
                    (subject.lower(),),
                ).fetchall()
                if not rows:
                    continue
                ...
                for relation, obj in rows:
                    if obj is None:
                        continue
                    relation = str(relation).replace("_", " ").lower()
                    if kind == "definition" and relation not in {
                        "defined as", "isa", "is a", "hypernym", "definition"
                    }:
                        continue
    ...
    def select(self, frame: Frame, user_text: str) -> tuple[str, str]:
        ...
        if subject:
            memory_candidate = self._memory_candidate(subject, kind)
            if memory_candidate:
                ...
                return memory_candidate, "knowledge"
        ...
        if teacher_candidate:
            ...
            if teacher_candidate.strip():
                ...
                return teacher_candidate.strip(), "participant"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="V513 cognitive bridge assistant")
    ...
    print("V513 cognitive bridge assistant ready.")
