...
def main():
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/v281"),
    )
    p.add_argument(
        "--dataset-output",
        type=Path,
        default=Path("results/v281_memory_read_dataset.jsonl"),
    )
    args=p.parse_args()
    ...
    print(
        "=== V281 MEMORY READ-PATH BENCHMARK ===",
        flush=True,
    )
    print(
        "architectures:",
    ...
    summary=args.output_dir/"v281_summary.json"
    summary.write_text(
