...
def entity_mention_from_parse(parse):
    ...
    # Pronouns are resolved by working memory, not by querying the graph for
    # literal words such as "it", "he", "she", etc.
    if lowered & Context.PRONOUNS:
        return None
    ...
    # Prefer an actual object in "of X" constructions.
    if parse.objects:
...
def trace_result(text, parse, hs, selected, search_result, ctx):
    # V626: keep the trace serializer on the current context-aware API.
    return {
        "timestamp": time.time(),
        "text": text,
        "parse": asdict(parse),
...
class SemanticResultRealizer:
    """
    Dual-mode local LLM surface.

    Grounded mode:
        verbalizes only a verified semantic result.

    Conversation mode:
        handles ordinary conversation when the semantic graph has no verified
        answer.
    """
    ...
    def build_grounded_packet(self, question, selected, result):
        evidence = []
        ...
        relation = selected.relation if selected is not None else ""

        return {
            "question": self._clean(question, 280),
            "subject": self._clean(
                selected.subject if selected is not None else "unknown",
                240,
            ),
    ...
    def build_grounded_prompt(self, question, selected, result):
        packet = self.build_grounded_packet(
            question,
            selected,
            result,
        )
        ...
        return (
            ...
            "The semantic system has already verified the fact.\n"
            "Use only the supplied fact packet.\n"
            "Do not use outside knowledge.\n"
            "Do not add facts, background, dates, places, people, institutions, "
            "descriptions, causes, or history.\n"
            "You may change wording and grammar, but preserve the verified "
            "meaning and object.\n"
    ...
    def build_conversational_prompt(self, question, history=None):
        ...
        for turn in history[-6:]:
            ...
            assistant = self._clean(
                turn.get("assistant", ""),
                300,
            )

            if user:
                recent.append("USER: " + user)
            if assistant:
                recent.append("ASSISTANT: " + assistant)
        ...
        return (
            ...
            "You can greet people, tell jokes, answer ordinary conversational "
            "requests, brainstorm, explain general ideas, and be friendly.\n"
    ...
    def _surface_form(value):
        return str(value).replace("_", " ").rstrip("/")
    ...
    def _object_grounded(cls, answer, target):
        ...
        return bool(terminal and terminal in answer_norm)
    ...
    @classmethod
    def grounded_ok(cls, answer, selected, result):
        return (
            ...
            and cls._relation_grounded(
                answer,
                selected.relation if selected is not None else "",
            )
        )

    @staticmethod
    def grounded_fallback(selected, result):
        target = result.get("target")

        if target is None:
            return "I don't have a verified answer for that."

        relation = selected.relation if selected is not None else ""
        label = SemanticResultRealizer._relation_label(relation)
        target_text = SemanticResultRealizer._surface_form(target)
        ...
        return "The verified result is " + target_text + "."

    def realize_grounded(self, question, selected, result):
        ...
        if not answer:
            return self.grounded_fallback(
                selected,
                result,
            )

        if not self.grounded_ok(
            answer,
            selected,
            result,
        ):
    ...
    def realize_conversation(self, question, history=None):
        ...
        answer = self._generate(
            self.build_conversational_prompt(
                question,
                history,
            )
        )
    ...
    def realize(self, question, selected, result, history=None):
        if (
            result.get("success", False)
            and selected is not None
            and selected.relation
            and not result.get("intent_only", False)
        ):
            return self.realize_grounded(
                question,
                selected,
                result,
            )

        return self.realize_conversation(
            question,
            history,
        )
...
def main():
    ...
    print("=== V626 SEMANTIC CHAT GATEWAY ===", flush=True)
    ...
    try:
        ...
        print(
            "LLM routing            : GROUNDED + CONVERSATION",
            flush=True,
        )
        ...
        if args.mode == "smoke":
            ...
            ctx = memory
            print("=== V626 CHAT (type 'exit') ===", flush=True)
            ...
            while True:
                ...
                if args.no_llm:
                    if semantic_answer:
                        ...
                        realized_answer = conversational_response(
                            user_text,
                            selected,
                            result,
                        )
                ...
                trace["llm_interface"] = (
                    compact_answer_evidence(result)
                    if semantic_answer
                    else {
                        "llm_called": False,
                        "reason": (
                            "conversation_or_nonsemantic_turn"
                        ),
                    }
                )
        ...
        print()
        print("=== V626 COMPLETE ===", flush=True)
