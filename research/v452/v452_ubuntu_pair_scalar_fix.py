...
def next_curiosity(con):
    """
    Prefer important assistant-facing node types and whichever relation family
    has the weakest direct evidence. Resource ingestion gives structure, but
    does not mark the family learned.
    """
    weights={
        "intent":3.5,
        "service":3.2,
    ...
    for row in con.execute("""
        ...
        FROM nodes
        WHERE node_type IN
          ('intent','service','slot','action','domain',
           'predicate','concept','dataset')
        ORDER BY count DESC
        LIMIT 5000
    """):
        node_id,node_type,label=row
        base=weights.get(node_type,1.0)

        # Keep a target-history lookup so failed attempts also count as asked.
        for family in QUESTION_FAMILIES:
            asked=con.execute("""
                SELECT COUNT(*) FROM curiosity_targets
                WHERE node_id=? AND family=?
            """,(node_id,family)).fetchone()[0]
...
def main():
    ap=argparse.ArgumentParser(
        description=(
            "Parse assistant dialogue datasets into a semantic net, "
            "then drive curiosity from missing assistant knowledge."
        )
    )
    ...
    ap.add_argument(
        "--parse-corpus-text",
        action="store_true",
        help="Parse every corpus utterance with spaCy. Off by default for speed.",
    )
    ap.add_argument(
        "--parser-model",
    ...
    ap.add_argument(
        "--rebuild-net",
        action="store_true",
        help="Delete and rebuild assistant_semantic_net.sqlite.",
    )
    ap.add_argument(
        "--curiosity-only",
        action="store_true",
        help="Skip source ingestion and use the existing semantic net.",
    )
    ap.add_argument(
        "--smoke",
        action="store_true",
    ...
    if args.rebuild_net and db_path.exists():
        db_path.unlink()
    ...
    print("="*78,flush=True)
    print(
        "V436 ASSISTANT SEMANTIC NET + CURIOSITY",
        flush=True,
    )
    print("="*78,flush=True)
    ...
    if not args.curiosity_only:
        print(
            "[1/6] Preparing fast corpus ingestion...",
            flush=True,
        )
        nlp=None
        parser_batch_size=16

        if args.parse_corpus_text:
            print(
                f"      full corpus parsing=ON model={args.parser_model}",
                flush=True,
            )
            nlp=load_parser(args.parser_model)
            parser_batch_size=configure_parser(nlp)
        else:
            print(
                "      full corpus parsing=OFF; "
                "using dataset-provided semantic annotations",
                flush=True,
            )

        print(
            "[2/6] Ingesting Schema-Guided Dialogue...",
            flush=True,
        )
        sgd=ingest_sgd(
            con,
        ...
        # DialoGLUE intentionally removed. SGD + MultiWOZ + Ubuntu provide
        # the assistant-facing dialogue/computer-language coverage we need.
        print(
            "[4/5] DialoGLUE skipped (not required)...",
            flush=True,
        )
        print(
            "      using SGD + MultiWOZ + Ubuntu for dialogue/assistant language",
            flush=True,
        )

        print(
            "[4/5] Ingesting Ubuntu pickle data...",
            flush=True,
        )
        ubu=ingest_ubuntu(
            con,
        ...
        print(
            "[1-5/6] Using existing semantic net...",
            flush=True,
        )
    ...
    print(
        "[5/5] Starting curiosity learner...",
        flush=True,
    )

    if nlp is None:
        print(
            f"      loading spaCy parser={args.parser_model} for curiosity...",
            flush=True,
        )
        nlp=load_parser(args.parser_model)
    ...
    if args.teacher_probe:
        print(
            f"      probing {args.teacher_probe} targets",
            flush=True,
        )

        probe_targets=[]
        probe_exclusions=set()

        for _ in range(
            args.teacher_probe
        ):
            t=next_curiosity(con)
            if not t:
                break
            probe_targets.append(t)
            probe_exclusions.add(
                (
                    t["node_id"],
                    t["family"],
                )
    ...
    q_path=RESULTS/"v452_curiosity_questions.jsonl"
    f_path=RESULTS/"v452_curiosity_failures.jsonl"
    report_path=RESULTS/"v452_assistant_curiosity_report.json"

    for concept_round in range(
        args.max_concepts
    ):
        round_targets=[]

        for _ in range(args.rounds):
            target=next_curiosity(con)
            if not target:
                break

            # Never repeat a target/family after it was already attempted.
            target_id=store_target(
                con,
                target,
            )
            ...
            if (
                ...
                stored=report_counts(con)

                print(
                    f"      PROGRESS questions={questions:,} "
                    f"accepted={accepted:,} "
                    f"failed={failed:,} "
                    f"new_facts={facts:,} "
                    f"nodes={stored['nodes']:,} "
                    f"edges={stored['edges']:,} "
                    f"curiosity_facts="
                    f"{stored['curiosity_answers']:,} "
                    f"rate={rate:.2f}/s",
                    flush=True,
                )

        if not round_targets and not target:
            break
    ...
    report={
        "status":"PASS"
        if counts["curiosity_answers"]>0
        else "FAIL",
        "version":"v452",
        "methodology":{
            "semantic_net_first":True,
            "sources":[
                "Schema-Guided Dialogue",
                "MultiWOZ",
                "DialoGLUE",
            ...
            "parser":"spaCy en_core_web_trf",
            "teacher":"SmolLM2-1.7B-Instruct",
            "curiosity_controller":"cognitive semantic net",
            "teacher_controls_curiosity":False,
            "failed_questions_count_as_attempted":True,"dialoglue_removed":True,"dialogue_sources":["SGD","MultiWOZ","Ubuntu"],"corpus_discovery":"automatic","bulk_transformer_parsing":"disabled by default","dialoglue_layout":"dataset-list aware","ubuntu_encoding":"automatic vocabulary discovery + ID decoding",
        },
        "paths":{
            "dstc8":DSTC8,
            "multiwoz":MULTIWOZ,
            "dialoglue":DIALOGUE,
            "ubuntu":UBUNTU,
            "semantic_net":db_path,
            "teacher_model":MODEL,
        },
        "curiosity":{
            "outer_iterations":args.max_concepts,
            "rounds_per_iteration":args.rounds,
            "questions":questions,
            "accepted":accepted,
            "failed":failed,
            "facts":facts,
