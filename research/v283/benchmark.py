...
def eligibility_loss(
    ...
    """
    Eligibility-style credit assignment.

    Earlier post-instruction logits receive exponentially decayed copies of
    the terminal target. This shortens the effective gradient path without
    forcing exact intermediate-state classification.
    """
    if item["horizon"]<=1:
        return torch.zeros((),device=device)
...
def td_value_loss(
    ...
    """
    TD-style auxiliary objective over terminal action confidence.

    We bootstrap a scalar continuation value from one timestep to the next.
    The implementation is deliberately simple: terminal correctness is the
    reward and intermediate steps learn a discounted target derived from the
    ...
    terminal_target=torch.ones_like(
        values[-1]
    )
...
def transition_supervision_loss(
    ...
    """
    Supervise that the post-step workspace continues to be predictive of the
    terminal action, but through a representation-preserving transition term.
    """
    if item["horizon"]<=1:
        return torch.zeros((),device=device)
    ...
    for t in range(1,item["horizon"]):
        # Preserve the direction of the memory update while attaching the
        # terminal target to its consequence.
        ...
        score=outputs[t]["action_logits"]

        # A small norm regularizer prevents the transition loss from being
        # satisfied by arbitrary magnitude growth.
        action_loss=F.cross_entropy(
            score[None],
            target,
        )
        transition_reg=(
            normalized_delta.norm(dim=-1)
            - 1.0
        ).abs().mean()

        values.append(
            action_loss+0.05*transition_reg
        )
