...
def load_conceptnet_stats(path: Path):
    """Read lightweight ConceptNet statistics without assuming a fixed schema."""
    import sqlite3
    ...
    try:
        ...
        # Support common compact ConceptNet schemas.
        for table in tables:
            cols = [
                r[1] for r in cur.execute(f'PRAGMA table_info("{table}")')
            ]
            lowered = {c.lower(): c for c in cols}
            if result["concepts"] == 0 and any(
                x in lowered for x in ("concept", "term", "node", "uri")
            ):
                try:
                    c = next(
                        lowered[x] for x in ("concept", "term", "node", "uri")
                        if x in lowered
                    )
                    result["concepts"] = int(
                        cur.execute(
            ...
            if result["edges"] == 0 and len(cols) >= 2:
                try:
                    result["edges"] = int(
                        cur.execute(
...
def conceptnet_matches(path: Path, terms):
    """
    Best-effort semantic lookup against compact ConceptNet. We only use this
    for grounding evidence; absence of a match never invalidates grammar.
    """
    import sqlite3
    ...
    try:
        ...
        for term in terms:
            ...
            for table in tables:
                ...
                candidate_cols = [
                    lc[x] for x in ("concept","term","node","uri")
                    if x in lc
                ]
                ...
                try:
                    rows = cur.execute(
                        ...
                        f'WHERE lower("{col}") LIKE ? LIMIT 8',
                        (f"%{term}%",)
                    ).fetchall()
                    matches.extend(str(r[0]) for r in rows)
                except Exception:
                    continue
            out[term] = list(dict.fromkeys(matches))[:8]
...
def main():
    ...
    semantic_matches=conceptnet_matches(conceptnet_path,sorted(semantic_terms))
    ...
    outputs={
        ...
        "cognitive_observations":results/"v422_cognitive_observations.jsonl",
        "failures":results/"v422_teacher_failures.jsonl",
        "candidates":results/"v422_quality_candidates.jsonl",
        "report":results/"v422_cognitive_distillation_report.json",
    }
    ...
    report={
        ...
        "cognitive_architecture":{
            ...
            "conceptnet":{
                ...
                "matched_terms":sum(
                    1 for v in semantic_matches.values() if v
                ),
            },
        },
        "checks":{
            ...
            "teacher_target_examples":teacher_exact_target>0,
