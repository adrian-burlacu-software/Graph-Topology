...
class Graph:
    ...
    def definition(
        ...
        # Full ConceptNet nodes generally do not have definitions. If the
        # lexical node is backed by a WordNet sense, surface the first
        # WordNet gloss attached to that concept.
        row = self.conn.execute(
            """
            SELECT n.definition
            FROM edges e
    ...
    if structural_concept_question(
        ...
        if concept:
            ...
            if resolution["status"] == "resolved":
                ...
                candidates = graph.relation_vocab(
                    max(
                        64,
                        max_n * 8,
                    )
                ...
                for relation in candidates:
                    ...
                    # The compact graph may not expose a useful phrase for a
                    # relation, so exact lexical/structural compatibility is
                    # the primary prior and graph proof is evaluated later.
                    scored.append(
                        (
                            score,
                            relation,
