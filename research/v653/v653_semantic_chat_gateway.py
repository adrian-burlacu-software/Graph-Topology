...
from v653_semantic_core import (
    Graph,
    DistilledMemory,
    candidate_senses,
    Context,
...
class Realizer:
    ...
    def grounded_prompt(
        ...
        return (
            "Write one short answer using ONLY the verified fact.\n"
            "Do not invent, substitute, or introduce another entity.\n"
            "Keep the verified subject, relation, and target unchanged.\n\n"
            f"QUESTION: {question}\n"
            f"VERIFIED_SUBJECT: {hypothesis.subject}\n"
            f"VERIFIED_RELATION: {hypothesis.relation}\n"
            f"VERIFIED_TARGET: {result.get('target')}\n"
            "ANSWER:"
        )
...
class LiveSemanticTeacher:
    ...
    def choose(self, question, candidates, decision_type, descriptions=None):
        import json
        import re
        ...
        prompt=(
            ...
            "Judge the relation meaning and target together.\n"
            "Prefer a specific/direct answer over a generic association.\n"
            "The candidates are the complete available choices.\n"
