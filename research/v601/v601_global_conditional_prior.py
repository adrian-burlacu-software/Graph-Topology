...
"""
V601 — GLOBAL CONDITIONAL PATH PRIOR (THREAD-SAFE)
...
class Graph:
    def __init__(self, path: Path, cache_entries: int = 12000) -> None:
        ...
        # SQLite connections are deliberately thread-local.  V600 shared one
        # connection between ThreadPoolExecutor workers, which can raise
        # sqlite3.InterfaceError even with check_same_thread=False.  V601
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default=r".\results\v601_global_conditional_prior.json",
    )
