...
from v677_offline_learning import worker_main
from v677_memory import SharedCheckpoint


def build_parser():
    ap = argparse.ArgumentParser(description="V677 19 offline workers + 1 online semantic chat worker")
    ap.add_argument("--database", required=True)
    ap.add_argument("--output", default="./results/v677_chat.json")
    ap.add_argument("--trace-output", default="./results/v677_chat_traces.jsonl")
    ap.add_argument("--memory-output", default="./results/v677_memory.json")
    ap.add_argument("--worker-log-dir", default="./results/v677_workers")
    ap.add_argument("--shared-memory", default="./results/v677_shared_memory.sqlite")
...
def launch_offline(args, stop_event):
    ...
    for worker_id in range(19):
        p = mp.Process(target=offline_process_entry, args=(child_args, worker_id, stop_event), name=f"v677-offline-{worker_id:02d}")
        p.daemon = True
        p.start()
        processes.append(p)
    return processes
...
def offline_process_entry(args, worker_id, stop_event):
    try:
        ...
        print(f"[V677 offline worker {worker_id}] FATAL: {exc!r}", flush=True)


def run_chat(args, stop_event):
    from v677_semantic_chat_gateway import run_chat_worker
...
def main():
    ...
    print("=== V677 COGNITIVE RUNTIME ===", flush=True)
    ...
    try:
        ...
        for p in offline:
            p.join(timeout=1.0)
        print("=== V677 RUNTIME COMPLETE ===", flush=True)
