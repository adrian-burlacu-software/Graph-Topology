...
def main():
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/v282"),
    )
    p.add_argument(
        "--dataset-output",
        type=Path,
        default=Path("results/v282_memory_read_dataset.jsonl"),
    )
    args=p.parse_args()
    ...
    print(
        "=== V282 MEMORY READ-PATH BENCHMARK ===",
        flush=True,
    )
    print(
        "architectures:",
    ...
    summary=args.output_dir/"v282_summary.json"
    summary.write_text(
