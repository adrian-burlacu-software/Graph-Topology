...
class BridgeAssistant:
    ...
    def respond(self,text):
        ...
        if not self.memory.knowledge_is_frozen():
            ...
            if terms:
                try:
                    raw_evidence=self.memory.static_facts(
                        terms,goal.name,domains=None,limit=24
                    )
                    evidence=select_llm_evidence(raw_evidence, target, max_items=4)
                except Exception:
                    evidence=[]
        ...
        frame=CognitiveFrame(
            ...
            evidence=compact_evidence(evidence, target)[:4],
