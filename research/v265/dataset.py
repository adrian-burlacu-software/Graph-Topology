...
def make_case(
    ...
    return {
        ...
        "horizon":horizon,
        "trajectory_states":[
            s.signature() for s in states
        ],
        "trajectory_actions":[
            action_record(a) for a in actions
        ],
        "goal":{
            "task":"memory",
            "source":"alpha",
...
def make_dataset(
    ...
    for horizon in (1,2,3,4):
        for i in range(pairs_per_horizon):
            ...
            # H1 intentionally has no delay: the instruction is available
            # at the decision. H2-H4 remove it after the initial step.
