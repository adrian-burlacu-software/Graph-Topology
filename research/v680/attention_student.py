...
class NeuralAttentionPolicy(nn.Module):
    def __init__(self, hidden_size=32, recurrent=True):
        super().__init__()
    ...
    def forward(self, state_vector, candidate_vectors, hidden=None, action_mask=None):
        ...
        candidate_logits = self.candidate_head(torch.cat([
            hidden.expand(candidate_embeddings.shape[0], -1), candidate_embeddings
        ], dim=-1)).squeeze(-1)
    ...
    def distribution(self, state, hidden=None, deterministic=False, recurrent=None):
        ...
        logits, value, hidden = self(
            state_vector, candidate_vectors, hidden, self.action_mask(state)
        )
    ...
    def select_action(self, state, candidates=None, deterministic=False, hidden=None):
        ...
        logits, distribution, index, value, hidden = self.distribution(
            state, hidden=hidden, deterministic=deterministic
        )
