...
from v672_memory import RamSemanticMemory, SharedCheckpoint
...
def run_lane(conn, ram, lane, seed, worker_id, batch_no):
    ...
    if lane in RELATION_GROUPS and RELATION_GROUPS[lane]:
        ...
        # V671 generated an uncontrolled Cartesian product. V672 deliberately
        ...
        max_derived = max(1, int(getattr(ram, "composition_max", 250)))
        produced = 0
        for subject in subjects:
            if produced >= max_derived:
                break
...
def worker_main(args, worker_id: int, stop_event=None):
    ...
    ram.composition_max = int(getattr(args, "composition_max", 250))
...
def build_parser():
    ...
    ap.add_argument("--composition-max", type=int, default=250)
    return ap
