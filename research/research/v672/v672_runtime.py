...
from v672_offline_learning import worker_main
from v672_memory import SharedCheckpoint


def build_parser():
    ap = argparse.ArgumentParser(description="V671 19 offline workers + 1 online semantic chat worker")
    ap.add_argument("--database", required=True)
    ap.add_argument("--output", default="./results/v672_chat.json")
    ap.add_argument("--trace-output", default="./results/v672_chat_traces.jsonl")
    ap.add_argument("--memory-output", default="./results/v672_memory.json")
    ap.add_argument("--worker-log-dir", default="./results/v672_workers")
    ap.add_argument("--shared-memory", default="./results/v672_shared_memory.sqlite")
...
def launch_offline(args, stop_event):
    ...
    for worker_id in range(19):
        p = mp.Process(target=offline_process_entry, args=(child_args, worker_id, stop_event), name=f"v672-offline-{worker_id:02d}")
        p.daemon = True
        p.start()
        processes.append(p)
    return processes
...
def run_chat(args, stop_event):
    from v672_semantic_chat_gateway import run_chat_worker
