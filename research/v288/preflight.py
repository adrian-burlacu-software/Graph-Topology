...
def main():
    ...
    p.add_argument("--pairs-per-horizon",type=int,default=30)
    p.add_argument("--seed",type=int,default=287)
    ...
    assert len(rows)==args.pairs_per_horizon*2

    pairs=collections.defaultdict(list)
    for row in rows:
        pairs[row["pair_id"]].append(row)

    assert len(pairs)==args.pairs_per_horizon
    ...
    assert len(ARCHITECTURES)*3==6

    print(
        "=== V288 H4-ONLY PREFLIGHT ===",
        flush=True,
    )
    print(
        "architectures=2 horizon=H4 seeds=3 cells=6",
        flush=True,
    )
    print(
        f"pairs={args.pairs_per_horizon}",
        flush=True,
    )
