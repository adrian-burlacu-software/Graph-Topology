...
def trace_result(text, parse, hs, selected, search_result, ctx):
    # V618: keep the trace serializer on the current context-aware API.
    return {
        "timestamp": time.time(),
        "text": text,
        "parse": asdict(parse),
...
def main():
    ...
    print("=== V618 SEMANTIC CHAT GATEWAY ===", flush=True)
    ...
    try:
        ...
        if args.mode == "smoke":
            ...
            ctx = memory
            print("=== V618 CHAT (type 'exit') ===", flush=True)
        ...
        print()
        print("=== V618 COMPLETE ===", flush=True)
