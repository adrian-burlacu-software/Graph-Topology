...
def ingest_ubuntu(con,paths_dataset,paths_vocab):
    ...
    for file_index,path in enumerate(paths_dataset,1):
        ...
        print(
            f"        decoded_relational_pairs={len(rows)} "
            f"vocab_tokens={len(vocab)}",
            flush=True,
        )

        # Token vocabulary is preserved explicitly, not inferred away.
        if vocab:
        ...
        for pair in rows:
            pair_key=f"{pair['container_index']}:{pair['row_index']}"
            sid=source_id(
                con,
                "ubuntu",
                path,
                ...
                "ubuntu_pair",
                {
                    "container_index":pair["container_index"],
            ...
            con.execute(
                ...
                INSERT OR IGNORE INTO ubuntu_pairs(
                    source_path,row_index,label_text,
            ...
            pair_row=con.execute(
                ...
                WHERE source_path=? AND row_index=?
                """,
                (str(path),pair["row_index"]),
            ).fetchone()
            ...
            # Preserve the actual relation represented by c -> r.
            if pair["user"] and pair["reply"]:
                ...
                con.execute(
                    ...
                    VALUES(
                        (SELECT concept_id FROM concepts WHERE canonical=? AND concept_type='concept'),
                        'next_reply',
                        (SELECT concept_id FROM concepts WHERE canonical=? AND concept_type='concept'),
                        NULL,'dialogue',?,?,?
                    )
                    """,
                    (
                        canonical(pair["user"][:200]),
                        canonical(pair["reply"][:400]),
                        sid,
                        0.65,
                        1.0,
                    ),
