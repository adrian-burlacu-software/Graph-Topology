...
class TypedMemory:
    ...
    def static_facts(self,subject_terms,goal_name,domains=None,limit=24):
        ...
        prefs={
            "request_information":{"semantic":1.0,"lexical":0.9,"dialogue":0.3,"procedural":0.4},
            "request_explanation":{"semantic":1.2,"lexical":0.8,"dialogue":0.3,"procedural":0.5},
            "request_generation":{"procedural":1.1,"dialogue":0.8,"semantic":0.3,"lexical":0.5},
            "challenge_claim":{"semantic":1.1,"lexical":0.5,"dialogue":0.5,"procedural":0.4},
            "explore_assistant":{"semantic":0.3,"lexical":0.3,"dialogue":0.7},
        }.get(
            goal_name,
            {"semantic":1.0,"lexical":0.8,"dialogue":0.5,"procedural":0.5},
        )

        rows=[]
        for term in list(dict.fromkeys(subject_terms))[:12]:
            sql="""
                SELECT c.canonical AS subject,
                       f.predicate,
            ...
            sql+="""
                GROUP BY c.canonical,f.predicate,
                         COALESCE(o.canonical,f.object_text),
                         f.fact_type,f.domain
                LIMIT ?
            """
            params.append(limit)
            rows.extend(
                dict(r) for r in self.con.execute(sql,params)
            )
