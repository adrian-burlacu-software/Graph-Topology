...
class AssociativeEdgeCell:
    ...
    def reward(self, value: float):
        ...
        Positive reward reinforces the association; negative reward weakens
        it. Strength remains bounded in [0, 1].
        """
        self.strength = max(
            0.0,
            min(
                1.0,
                self.strength + self.learning_rate * value,
            ),
        )


class EdgeCellV4(DualVocabularyV6):
    """
    EdgeCell V2.
    ...
    def reward_edge(self, word, pos, reward):
        key = self.edge_key(word, pos)
        if key is None:
            return
        cell = self.edge_cells.get(key)
    ...
    def train_edges_with_reward(self, words, epochs=5):
        print("=== EDGECELL V4 REWARD PLASTICITY TRAINING ===")
        print()
        ...
        for epoch in range(1, epochs + 1):
            ...
            for word in words:
                ...
                for pos in range(len(word)):
                    # Structural association is learned first.
                    self.learn_edge(word, pos)

                    action = self.designer_from_edge_cell(
                        word, pos, learn=False
                    )

                    # Correct structural association gets positive reward.
                    # This branch is intentionally simple: the experiment
                    # isolates reward-modulated edge plasticity.
                    if action == "REUSE":
                        reuse += 1
                        self.reward_edge(word, pos, +1.0)
                    else:
                        branch += 1
                        self.reward_edge(word, pos, +0.25)
    ...
    def print_edge_cells(self):
        print()
        print("=== ASSOCIATIVE EDGE CELLS V4 ===")
...
def run():
    net = EdgeCellV4(deepcopy(GENOME))
    ...
    print("Edge cells now support reward-modulated plasticity.")
    print("Edge learning is direct bounded accumulation; threshold is 0.75.")
    print()

    net.train_edges_with_reward(TRAINING, epochs=5)
