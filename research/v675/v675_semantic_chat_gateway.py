...
from v675_semantic_core import (
    Graph,
    DistilledMemory,
    candidate_senses,
    Context,
...
from v675_memory import RamSemanticMemory, SharedCheckpoint, SharedDistilledMemory
...
def run_chat_worker(args):
    ...
    ap.add_argument(
        "--shared-memory",
        required=False,
        default="./results/v675_shared_memory.sqlite",
    )
    ap.add_argument(
        "--worker-id",
        type=int,
    ...
    # args is supplied by the V675 runtime
        ...
        while not sync_stop.wait(0.50):
            try:
                ...
                try:
                    ...
                    pass
    sync_thread = threading.Thread(target=_sync_loop, name="v675-chat-checkpoint", daemon=True)
    sync_thread.start()
...
def main():
    # Standalone compatibility mode: V675 chat worker without the 19-worker runtime.
    ap = argparse.ArgumentParser()
