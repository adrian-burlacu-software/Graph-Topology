...
class Context:
    ...
    def subject(
        ...
        # Conversation context takes precedence over graph entity lookup.
        ...
        if mention:
            if graph is not None:
                ...
                # Never promote a fuzzy candidate.
                return self.active_subject
...
def entity_mention_from_parse(parse):
    reference_pronouns={"it","he","she","they","him","her","them","this","that","these","those","its","his","their"}
    wh_words={"who","what","where","when","which","whom","whose","why","how"}
    lowered={token["text"].lower() for token in parse.tokens}
    if lowered & reference_pronouns:
        return None
    uri=extract_graph_mentions(parse.text)
    if uri:
        return uri[0]
    for entity in parse.entities:
        value=str(entity.get("text","")).strip()
        if value and value.lower() not in wh_words:
            return value
    if parse.question=="WH_WHAT" and parse.root_lemma in {"be","mean","refer"} and not parse.entities:
        return None
    for value in list(parse.objects)+list(parse.subjects):
        words=[w for w in str(value).strip().split() if w.lower() not in wh_words and w.lower() not in reference_pronouns]
        if words: return " ".join(words)
    for chunk in getattr(parse,"noun_chunks",[]):
        words=[w for w in str(chunk).split() if w.lower() not in wh_words and w.lower() not in reference_pronouns]
        if words: return " ".join(words)
    return None
...
def _intent_hypotheses(
    ...
    has_interjection = "INTJ" in pos
    concept_question = (
        parse.question == "WH_WHAT"
        and parse.root_lemma in {"be","mean","refer"}
        and not has_entity
    )
    pronoun_context = (
        any(
    ...
    # Generic spaCy structural evidence.
    if has_interjection:
        scores["conversation"] += 0.60

    if concept_question:
        scores["conversation"] = 1.0
        scores["relation_lookup"] = 0.0
        scores["identity_lookup"] = 0.0
        scores["entity_profile"] = 0.0
        scores["statement"] = 0.0
    elif has_question:
    ...
    if pronoun_context:
        scores["relation_lookup"] += 0.20
        scores["entity_profile"] += 0.10

    # A declarative noun phrase without an interjection/question should not
    # receive a blanket "conversation" win.
    if not has_interjection and not has_question:
        if has_entity or has_subject or has_object:
            scores["statement"] += 0.20
        elif not pronoun_context:
            scores["conversation"] += 0.05
...
def is_general_concept_question(
    ...
    if parse.question != "WH_WHAT":
        return False

    if parse.root_lemma not in {
        "be",
        "mean",
        "refer",
    }:
    ...
    # A named/N recognizer entity means this is not a generic concept.
    if "has_entity" in features:
        return False

    # "what" itself is the WH operator, not an entity.
    resolution = evidence.get(
        "entity_resolution",
        {},
    )
    if isinstance(
    ...
    # Structural noun/subject/object evidence is sufficient.
    return bool(
        getattr(
            parse,
            "subjects",
...
def trace_result(text, parse, hs, selected, search_result, ctx):
    # V629: keep the trace serializer on the current context-aware API.
    return {
        "timestamp": time.time(),
        "text": text,
        "parse": asdict(parse),
...
def smoke(
    ...
    for index, user_text in enumerate(
        ...
        trace["relation_attention"] = (
            relation_attention.export()
        )
        trace["path_attention"] = (
        ...
        realized_answer = (
            ...
            if not args.no_llm
                ...
                if result.get("success", False)
                else "I don't have a verified answer for that."
            )
        )
        ...
        trace["realized_answer"] = realized_answer
        ...
        print(
            ...
            f"direct="
            f"{result.get('direct_proof', False)} "
            f"steps="
            f"{result.get('steps', 0)}",
            flush=True,
        )
        print(
            "  entity resolution:",
        ...
        print(
            f"  relation_attention_updates="
            f"{relation_attention.updates} "
            f"path_attention_updates="
            f"{path_attention.updates}",
            flush=True,
        )
...
class SemanticResultRealizer:
    """
    Closed-world structured fact realizer.

    The cognitive runtime is authoritative. The LLM only turns a verified
    subject/relation/object fact into a natural-language sentence.
    """
    ...
    def _load(self):
        ...
        try:
            ...
            raise RuntimeError(
                "Local SmolLM3 realization requires transformers: "
                + str(exc)
            ) from exc
    ...
    def build_fact_packet(
        ...
        return {
            "question": self._clean(question, 280),
            "subject": self._clean(
                selected.subject
                if selected is not None
                else "unknown",
            ...
            "relation": self._clean(relation, 160),
            "relation_label": self._relation_label(
                relation
            ),
            "verified_object": self._clean(
    ...
    def build_prompt(
        ...
        return (
            ...
            "The semantic result is already verified.\n"
            "Use ONLY the fact packet below.\n"
            "Do not add facts from pretrained knowledge.\n"
            "Do not infer or guess what an identifier refers to.\n"
            "Do not add dates, places, people, institutions, descriptions, "
            "history, causes, or attributes.\n"
            "You may change grammar and wording only.\n"
            "Return exactly one concise natural-language sentence.\n\n"
            "QUESTION: "
            + packet["question"]
            + "\nSUBJECT: "
            + packet["subject"]
            ...
            + packet["relation"]
            + "\nRELATION LABEL: "
            + packet["relation_label"]
            + "\nVERIFIED OBJECT: "
            + packet["verified_object"]
            + "\nEVIDENCE PHRASES: "
            + evidence
            + "\nANSWER:"
        )
    ...
    @classmethod
    def _object_grounded(cls, answer, target):
        if target is None:
            return False

        answer_norm = str(answer).lower().replace("_", " ")
        target_norm = str(target).lower().replace("_", " ")

        if target_norm in answer_norm:
            return True

        terminal = target_norm.rstrip("/").split("/")[-1].split(":")[-1]
        return bool(
            terminal
            and terminal in answer_norm
        )

    @classmethod
    def _relation_grounded(cls, answer, relation):
        if not relation:
            return True

        answer_norm = str(answer).lower()
        relation_norm = str(relation).lower()

        if relation_norm in answer_norm:
            return True

        label = cls._relation_label(relation).lower()
        return bool(
            label
            and label in answer_norm
        )
    ...
    def _is_grounded(
        ...
        return (
            cls._object_grounded(
                answer,
                result.get("target"),
            )
            and cls._relation_grounded(
                answer,
                selected.relation
                if selected is not None
                else "",
    ...
    @staticmethod
    def _fallback_sentence(selected, result):
        target = result.get("target")
        if target is None:
            return (
                "I don't have a verified answer for that."
            )
        ...
        label = SemanticResultRealizer._relation_label(
            relation
        )
    ...
    def build_conversational_prompt(self, question, history=None):
        recent=[]
        for turn in (history or [])[-6:]:
            if isinstance(turn,dict):
                if turn.get("user"): recent.append("USER: "+self._clean(turn.get("user"),220))
                if turn.get("assistant"): recent.append("ASSISTANT: "+self._clean(turn.get("assistant"),280))
        return ("You are the conversational assistant.\n"
                "Respond naturally to the user. You may greet people, tell jokes, explain general concepts, brainstorm, and handle ordinary conversation.\n"
                "Do not claim that the semantic graph verified a fact when it did not.\n"
                "Be concise unless the user asks for detail.\n\n"
                "RECENT CONVERSATION:\n"+("\n".join(recent) if recent else "none")+
                "\nUSER: "+self._clean(question)+"\nASSISTANT:")

    def build_unresolved_prompt(self, question, result):
        resolution=result.get("entity_resolution",{})
        mention=resolution.get("mention") if isinstance(resolution,dict) else None
        return ("You are the answer layer for a semantic knowledge graph.\n"
                "The graph did NOT verify the requested entity or fact.\n"
                "Do NOT answer from pretrained knowledge.\n"
                "Do NOT guess or identify the entity yourself.\n"
                "State clearly that the semantic system could not verify the request.\n"
                "Return one concise sentence.\n\n"
                "USER QUESTION: "+self._clean(question,280)+
                "\nUNVERIFIED MENTION: "+self._clean(mention or "unknown",240)+"\nANSWER:")

    def realize_conversation(self, question, history=None):
        answer=self._generate(self.build_conversational_prompt(question,history),temperature=0.15)
        return answer or "I'm not sure how to respond to that."

    def realize_unresolved(self, question, selected, result):
        answer=self._generate(self.build_unresolved_prompt(question,result),temperature=0.10)
        markers=("couldn't verify","could not verify","cannot verify","can't verify","not verified","unable to verify","not found","unknown","unverified","couldn't confirm","could not confirm")
        if not answer or not any(m in answer.lower() for m in markers):
            return "I couldn't verify that from the semantic graph."
        return answer
    ...
    def realize(
        ...
        if not self.available:
            raise RuntimeError(
                "Local LLM model path does not exist: "
                + self.model_path
            )
        ...
        sampling_temperature = self.temperature if temperature is None else float(temperature)
        ...
        with torch.no_grad():
            generated = self._model.generate(
                **encoded,
                max_new_tokens=self.max_new_tokens,
                do_sample=sampling_temperature > 0.0,
                temperature=(sampling_temperature if sampling_temperature > 0.0 else 1.0),
                pad_token_id=self._tokenizer.pad_token_id,
                eos_token_id=self._tokenizer.eos_token_id,
            )
...
def compact_answer_evidence(result):
    return {
        "success": bool(
            result.get(
                "success",
...
def conversational_response(
    ...
    """
    Non-semantic conversational turns must not invoke the LLM realizer.

    The local LLM exists to verbalize verified semantic results. Greetings and
    other intent-only turns stay in the gateway and never trigger model load.
    """
...
def main():
    ...
    print("=== V629 SEMANTIC CHAT GATEWAY ===", flush=True)
    ...
    try:
        ...
        realizer = SemanticResultRealizer(
            args.llm_model,
        ...
        if args.mode == "smoke":
            ...
            ctx = memory
            print("=== V629 CHAT (type 'exit') ===", flush=True)
            ...
            while True:
                ...
                concept_question = is_general_concept_question(parse, selected)
                selected_evidence = selected.evidence if selected is not None and isinstance(selected.evidence, dict) else {}
                entity_resolution = selected_evidence.get("entity_resolution", {})
                if not isinstance(entity_resolution, dict): entity_resolution = {}
                top_resolution = result.get("entity_resolution", {})
                if isinstance(top_resolution, dict) and top_resolution: entity_resolution = top_resolution
                unresolved_entity = entity_resolution.get("status") in {"unresolved","ambiguous"} and bool(entity_resolution.get("mention"))
                if concept_question: unresolved_entity = False
                semantic_answer = bool(result.get("success",False)) and not bool(result.get("intent_only",False)) and bool(selected and selected.relation)
                if args.no_llm:
                    if semantic_answer:
                        realized_answer=str(result.get("target")); llm_mode="off_grounded"
                    elif unresolved_entity:
                        realized_answer="I couldn't verify that entity from the semantic graph."; llm_mode="off_unresolved"
                    else:
                        realized_answer=conversational_response(user_text,selected,result); llm_mode="off_conversation"
                elif semantic_answer:
                    realized_answer=realizer.realize(user_text,selected,result); llm_mode="grounded"
                elif unresolved_entity:
                    payload=dict(result); payload["entity_resolution"]=entity_resolution
                    realized_answer=realizer.realize_unresolved(user_text,selected,payload); llm_mode="unresolved_semantic"
                else:
                    realized_answer=realizer.realize_conversation(user_text,history=memory.turns if hasattr(memory,"turns") else []); llm_mode="conversation"
                trace["llm_mode"]=llm_mode
                trace["realized_answer"]=realized_answer
        ...
        print()
        print("=== V629 COMPLETE ===", flush=True)
        print(f"mode                  : {args.mode}", flush=True)
        print(
            "LLM realization         : "
            + ("OFF" if args.no_llm else "SmolLM3-3B"),
            flush=True,
        )
        print(
            "LLM model loaded        : "
            + (
                "YES"
                if realizer._model is not None
                else "NO"
            ),
            flush=True,
        )
