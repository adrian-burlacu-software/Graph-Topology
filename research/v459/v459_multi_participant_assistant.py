...
class NativeAssistant:
    """
    Multi-participant cognitive runtime.

    Participants:
      USER       = external speaker
      LLM        = internal language participant
      ARCHITECTURE= controller / perceiver / reasoner / memory

    The LLM's utterance is NEVER directly exposed to the user.
    """
    ...
    def reason(self,perception,hits):
        text_lower=perception["text"].strip().lower()
        ...
        request_words={
            "can","could","please","help","how","what","where",
            "when","why","show","find","get","open","move",
            "copy","delete","save","create","make","write",
        }

        request_like=any(
            token["text"].lower() in request_words
            for token in perception["tokens"]
        )
        ...
        if request_like and intent_hits:
    ...
    def choose_llm_question_family(self,perception,reasoning):
        if reasoning.get("decision")=="social":
            return (
                "conversation",
                "hello",
            )
        ...
        if perception["predicates"]:
            ...
            has_object=bool(perception["nouns"])
    ...
    def make_llm_question(self,perception,reasoning):
        ...
        if family=="conversation" and topic=="hello":
            return family,(
                "Someone says hello to you. "
                "What is a natural reply?"
            )

        template=self.QUESTION_FAMILIES[family]
        return family,template.format(topic=topic)

    def llm_speak(self,question,context):
    ...
    def perceive_llm(self,llm_message):
        """
        Treat LLM output as speech from another participant and perceive it
        exactly like user speech.
        """
        if not llm_message:
            return None
        ...
        llm_message["perception"]=perception
        return perception
    ...
    def compare_participants(
        ...
        """
        Architecture compares the two participants and makes a decision.

        The LLM does not get to choose the final response.
        """
        ...
        # Social turn: LLM provides a conversational candidate, which the
        # architecture accepts as a candidate response after perception.
        if reasoning["decision"]=="social":
            return {
                "decision":"social_reply",
                "confidence":8.0+min(2.0,overlap),
            }

        # If the LLM gives actionable/semantic material related to the user,
        # architecture can use it as an informed answer.
        if overlap>0:
            return {
                "decision":"llm_informed",
                "confidence":max(
        ...
        # Otherwise the architecture did not establish relevance.
        return {
            "decision":"native",
            "confidence":reasoning["confidence"],
        }

    def render_native(self,perception,reasoning):
        if reasoning["decision"]=="clarify_object":
            return (
                f"What would you like me to "
                f"{perception['predicates'][0]}?"
            )

        if reasoning["decision"]=="clarify":
            if perception["nouns"]:
                return (
                    f"What would you like to do with the "
                    f"{perception['nouns'][0]}?"
                )
            return (
                f"What would you like me to do with "
                f"{perception['predicates'][0]}?"
            )

        if reasoning["relations"]:
            seen=set()
            statements=[]

            for subject,relation,target in reasoning["relations"]:
    ...
    def native_reply_from_llm(
        ...
        """
        Convert the internal LLM participant's contribution into a response
        chosen by the architecture.

        The default is not to quote the LLM verbatim unless the architecture
        explicitly decides that the LLM's utterance is suitable as a reply.
        """
        if not llm_message:
            return None

        if participant_decision=="social_reply":
            # LLM's text is treated as a candidate conversational move.
            # It can be returned for social replies because the architecture
            # has explicitly classified it as such.
            return llm_message["text"]

        if participant_decision=="llm_informed":
            # For substantive turns, the architecture can synthesize a short
            # response from what it understood rather than exposing the
            # participant's raw utterance. Keep it simple and native.
            if (
                user_perception["nouns"]
                and llm_perception["nouns"]
            ):
                return (
                    "I understand. "
                    + llm_message["text"]
    ...
    def integrate_llm_knowledge(
        ...
        predicates=llm_perception["predicates"]
    ...
    def store_user_and_assistant_turns(
        self,
        turn_id,
        user_perception,
        reasoning,
    ...
    def respond(self,text):
        ...
        print(
            f"  [USER PERCEPTION] predicates="
            f"{user_perception['predicates']} "
        ...
        if self.teacher is not None:
            ...
            if llm_message:
                ...
                print(
                    f"  [ARCHITECTURE ← LLM] predicates="
                    f"{llm_perception['predicates']} "
                    f"nouns={llm_perception['nouns']}",
                    flush=True,
                )

                participant_decision=self.compare_participants(
                    user_perception,
                    llm_perception,
                    reasoning,
                )

                reasoning["participant_decision"]=participant_decision

                print(
                    f"  [ARCHITECTURE] decision="
                    f"{participant_decision['decision']} "
                    f"confidence="
                    f"{participant_decision['confidence']:.2f}",
                    flush=True,
                )

                llm_learned=self.integrate_llm_knowledge(
                    turn_id,
                    llm_message,
                    llm_perception,
                )

                reasoning["llm_learned"]=llm_learned

        response=self.native_reply_from_llm(
            user_perception,
            llm_message,
            llm_perception,
        ...
        self.store_user_and_assistant_turns(
            turn_id,
            user_perception,
            reasoning,
            response,
        ...
        return {
            "response":response,
            "decision":participant_decision["decision"],
            "confidence":participant_decision["confidence"],
            "retrieved":len(hits),
            "learned":reasoning.get("llm_learned",0),
