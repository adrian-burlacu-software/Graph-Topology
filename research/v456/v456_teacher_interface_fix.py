...
class NativeAssistant:
    """Native cognitive chat runtime; SmolLM2 is optional fallback."""
    ...
    def retrieve(self,perception):
        terms=list(dict.fromkeys(
            ...
            + perception["nouns"]
            + [e["lemma"] for e in perception["entities"]]
        ))
    ...
    def reason(self,perception,hits):
        request_words={
            ...
            "when","why","show","find","get","open","move","copy",
            "delete","save","create","make","write",
        }
        ...
        return {
            ...
            "confidence":min(
                10.0,
                support/3.0 + (2.0 if intent_hits else 0.0),
            ),
            "relations":[
                (
                    h["label"],
    ...
    def render(self,perception,reasoning):
        ...
        if reasoning["decision"]=="clarify":
            ...
            return f"What would you like me to do with {verb}?"
        ...
        if reasoning["relations"]:
            ...
            for subject,relation,target in reasoning["relations"]:
                ...
                seen.add(key)
                statements.append(
                    f"{subject} {relation.replace('_',' ')} {target}"
                )
                if len(statements)>=3:
                    break
        ...
        return (
            ...
            "Tell me what you're trying to do and I'll learn from it."
        )

    def learn(self,turn_id,perception,reasoning,response):
        learned=0

        for predicate in perception["predicates"]:
            subject=(
                perception["nouns"][0]
                if perception["nouns"] else ""
            )
            obj=(
                perception["nouns"][1]
                if len(perception["nouns"])>1 else ""
            )
    ...
    def maybe_teacher(self,perception,reasoning):
        if self.teacher is None:
            return None
        if reasoning["decision"]!="unknown":
            return None

        question=perception["text"]
        context={
            "known_terms":", ".join(
                perception["predicates"]+perception["nouns"]
            ),
    ...
    def respond(self,text):
        ...
        reasoning=self.reason(perception,hits)
        response=self.render(perception,reasoning)

        fallback=self.maybe_teacher(
            perception,
            reasoning,
        )
        if fallback:
            response=fallback
            reasoning["decision"]="teacher_fallback"
