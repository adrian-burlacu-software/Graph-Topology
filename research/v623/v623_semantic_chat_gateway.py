...
def trace_result(text, parse, hs, selected, search_result, ctx):
    # V623: keep the trace serializer on the current context-aware API.
    return {
        "timestamp": time.time(),
        "text": text,
        "parse": asdict(parse),
...
def main():
    ...
    print("=== V623 SEMANTIC CHAT GATEWAY ===", flush=True)
    ...
    try:
        ...
        if args.mode == "smoke":
            ...
            ctx = memory
            print("=== V623 CHAT (type 'exit') ===", flush=True)
        ...
        print()
        print("=== V623 COMPLETE ===", flush=True)
