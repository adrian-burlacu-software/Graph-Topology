...
class NativeAssistant:
    """
    Cognitive controller with an optional plain-English LLM partner.

    The architecture remains responsible for:
      - perception
      - memory retrieval
      - gap identification
    ...
    The LLM is treated as another conversational source of information. It is
    never asked to emit internal JSON or understand architecture terminology.
    """
    ...
    def reason(self,perception,hits):
        ...
        intent_hits=[
            h for h in hits
            if h["node_type"] in {"intent","action","service"}
        ]
        ...
        elif relation_count>0 and support>=self.confidence_threshold:
            decision="answer"
    ...
    def choose_llm_question_family(self,perception,reasoning):
        """
        Choose a plain-English question based on the architecture's gap.

        We deliberately keep this tiny and deterministic. The LLM never sees
    ...
    def ask_llm(self,question,context):
        ...
        # The teacher is asked in the same plain English an ordinary person
        # could understand. No JSON, schema names, or cognitive terminology.
        teacher_context={
            "user message":context.get("user_message",""),
            "known words":context.get("known_words",""),
            "known relations":context.get("known_relations",""),
        }
    ...
    def parse_llm_answer(self,answer):
        """
        Parse the LLM's ordinary English into explicit candidate facts using
        the existing spaCy parser. The teacher is not trusted to format data.
        """
        if not answer:
            return {
                "text":"",
    ...
    def integrate_llm_knowledge(
        ...
        """
        Store parser-derived interaction knowledge conservatively.

        We do not treat every word in the answer as truth. We only add explicit
        graph links when the answer contains a concrete predicate and at least
        one concrete noun term.
        """
        learned=0

        for predicate in parsed["predicates"][:8]:
            pnode=add_node(
                self.con,
                "predicate",
                predicate,
    ...
    def decide_from_llm(
        ...
        """
        Architecture-level decision after hearing the LLM.

        The LLM suggests. The architecture chooses.
        """
        if not parsed_answer["text"]:
            return "unknown"

        answer_predicates=set(
            parsed_answer["predicates"]
        )
        answer_nouns=set(
            parsed_answer["nouns"]
        )
        ...
        overlap=(
            len(answer_predicates & user_predicates)
            +len(answer_nouns & user_nouns)
        )

        if (
            reasoning["decision"]
            in {"clarify_object","clarify"}
        ):
            return "ask_user"

        if overlap>0 or answer_predicates or answer_nouns:
            return "llm_informed_reply"

        return "unknown"

    def render_llm_informed(
        self,
        perception,
        reasoning,
        llm_answer,
        ...
        if reasoning["decision"] in {
            "clarify_object",
            "clarify",
        }:
            return self.render_native(
                perception,
                reasoning,
            ) or llm_answer

        # We keep response realization native and use the LLM as an information
        # source. This avoids handing final conversational authority to the 1.7B.
        if llm_answer:
            return llm_answer
    ...
    def learn_interaction(
        ...
        for predicate in perception["predicates"]:
            subject=(
                perception["nouns"][0]
                if perception["nouns"]
                else ""
    ...
    def respond(self,text):
        ...
        native_response=self.render_native(
            perception,
            reasoning,
        )

        # The LLM is consulted as a peer whenever configured. It is not a
        # last-resort "unknown" answer generator.
        llm_answer=None
        llm_parsed={
            "text":"",
            "predicates":[],
        ...
        if self.teacher is not None:
            ...
            print(
                f"  [COGNITION] asking LLM family={family}: "
                f"{question}",
                flush=True,
            )
            ...
            if llm_answer:
                ...
                llm_parsed=self.parse_llm_answer(
                    llm_answer,
                )
                ...
                reasoning["llm_question"]=question
                reasoning["llm_family"]=family
                reasoning["llm_parsed"]=llm_parsed
                reasoning["llm_learned"]=llm_learned

                if reasoning["decision"]=="social":
                    final_decision="llm_informed_reply"
                else:
                    final_decision=self.decide_from_llm(
                        perception,
                        reasoning,
                        llm_parsed,
                    )
                reasoning["decision"]=final_decision

                if final_decision=="ask_user":
                    response=native_response
                else:
                    response=self.render_llm_informed(
                        perception,
                        reasoning,
                        llm_answer,
                        llm_parsed,
        ...
        learned=self.learn_interaction(
            turn_id,
            perception,
            reasoning,
            response,
