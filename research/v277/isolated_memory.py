...
def main():
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/v277"),
    )
    p.add_argument(
        "--dataset-output",
        type=Path,
        default=Path("results/v277_memory_read_dataset.jsonl"),
    )
    args=p.parse_args()
    ...
    print(
        "=== V277 MEMORY READ-PATH BENCHMARK ===",
        flush=True,
    )
    print(
        "architectures:",
    ...
    summary=args.output_dir/"v277_summary.json"
    summary.write_text(
