...
from v679_offline_learning import worker_main
from v679_memory import SharedCheckpoint
...
def build_parser():
    ap = argparse.ArgumentParser(description="V679 general-lane worker pool + online semantic chat worker")
...
def launch_offline(args, stop_event):
    ...
    for worker_id in range(args.worker_count):
        p = mp.Process(target=offline_process_entry, args=(child_args, worker_id, stop_event), name=f"v679-offline-{worker_id:02d}")
        p.daemon = True
        p.start()
        processes.append(p)
    return processes
...
def offline_process_entry(args, worker_id, stop_event):
    try:
        ...
        print(f"[V679 offline worker {worker_id}] FATAL: {exc!r}", flush=True)


def run_chat(args, stop_event):
    from v679_semantic_chat_gateway import run_chat_worker
...
def main():
    ...
    print("=== V679 COGNITIVE RUNTIME ===", flush=True)
    print(
    ...
    try:
        ...
        try:
            ...
            checkpoint.close()
        print("=== V679 RUNTIME COMPLETE ===", flush=True)
