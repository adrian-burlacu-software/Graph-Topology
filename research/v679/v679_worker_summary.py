...
def worker_summary(log_dir: Path):
    ...
    for path in sorted(log_dir.glob("worker_*.jsonl")):
        ...
        worker = {
            ...
            "new_results": int(latest.get("new_results", 0) or 0),
            "no_new_streak": int(latest.get("no_new_streak", 0) or 0),
        ...
        for key in ("batches", "items", "learned", "new_results", "errors", "syncs", "cpu_seconds",
...
def main():
    parser = argparse.ArgumentParser(
        description="Write a compact V679 overnight-run worker/checkpoint summary."
    )
    parser.add_argument("--worker-log-dir", default="./results/v679_workers")
    parser.add_argument("--shared-memory", default="./results/v679_shared_memory.sqlite")
    parser.add_argument("--output", default="./results/v679/worker_summary.jsonl")
