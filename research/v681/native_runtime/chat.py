...
def run_chat_worker(args):
    ...
    ap.add_argument(
        "--shared-memory",
        required=False,
        default="./results/v681/shared_memory.sqlite",
    )
    ap.add_argument(
        "--worker-id",
        type=int,
    ...
    attention_controller = AttentionController(policy=(
        ReloadingAttentionPolicy(args.policy_provider) if getattr(args, "policy_provider", None) else
        PromotedAttentionPolicy(policy_path) if policy_path.endswith(".pt")
        else DistilledAttentionPolicy.load(policy_path) if policy_path else None))
    ...
    try:
        checkpoint.sync(ram_memory, "chat", os.getpid(), force=True)
    except Exception:
        pass
        ...
        while not sync_stop.wait(0.50):
            try:
                if checkpoint.should_sync():
                    checkpoint.sync(ram_memory, "chat", os.getpid())
            except Exception as exc:
                try:
...
def main():
    ...
    ap.add_argument("--output", default="./results/v681/chat.json")
    ...
    ap.add_argument("--shared-memory", default="./results/v681/shared_memory.sqlite")
