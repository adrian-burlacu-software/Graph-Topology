...
def candidate_quality(verb,obj):
    """
    Score a construction for reusable semantic teaching value.
    This is a ranking signal, not the acceptance test for teacher output.
    """
...
def mine_candidates(sentences,max_candidates):
    ...
    for sent in sentences:
        for tok in sent.tokens:
            ...
            # Prefer concrete verb + object constructions.
            for obj in objects:
            ...
            # Bare actions are retained only when they are known useful
            # actions and have no usable direct object in the observed sentence.
    ...
    for (verb,obj,construction),freq in counts.items():
        ...
        score=math.log1p(freq)*q*max(0.55,1.0-0.008*len(construction))
        rows.append(
            Candidate(
                verb,
                obj,
    ...
    # Deduplicate by verb where possible so the teacher gets broad coverage
    # rather than ten variants of the same verb.
    selected=[]
    seen_verbs=set()
    deferred=[]
    ...
    # Fill remaining slots with the highest-scoring deferred constructions.
    for row in rows:
        if row in selected:
            continue
        deferred.append(row)
        if len(selected)+len(deferred)>=max_candidates:
            break
...
def sentence_prompt(candidate):
    if candidate.object_lemma:
        return (
            f'Write one short, normal English sentence. '
            f'Use the exact word "{candidate.verb}" and the exact word '
            f'"{candidate.object_lemma}" in the sentence. Do not explain.'
        )
    return (
        f'Write one short, normal English sentence. '
        f'Use the exact word "{candidate.verb}" in the sentence. Do not explain.'
    )
...
def smoke():
    ...
    print("single useful distillation path: PASS")
...
def main():
    ...
    print("[3/7] Loading SmolLM2...",flush=True)
    teacher=Teacher(args.model,args.max_new_tokens)

    print("[4/7] Loading independent parser...",flush=True)
    ...
    print("[5/7] Generating + parsing natural examples...",flush=True)
    ...
    print("[6/7] Building distillation summary...",flush=True)
    ...
    outputs={
        ...
        "failures":results/"v421_teacher_failures.jsonl",
        "candidates":results/"v421_quality_candidates.jsonl",
        "report":results/"v421_quality_distillation_report.json",
    }
