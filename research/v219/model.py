...
class CognitiveModel(nn.Module):
    ...
    @torch.no_grad()
    def autonomous_rollout(self, initial_state, goal, device, steps, transformer_depth=None):
        """Run the cognitive loop without teacher-supplied future states."""
        ...
        for _ in range(steps):
            out = self.cognitive_step(current, goal, working, depth, device)
            ...
            source = current.nodes[source_i].concept if source_i < len(current.nodes) else None
            target = current.nodes[target_i].concept if target_i < len(current.nodes) else None

            relation_id = int(out["relation_logits"].argmax().item())
            relation = next(
                (name for name, rid in RELATION_TO_ID.items() if rid == relation_id),
                None,
            )
            ...
            # NOOP/COMMIT terminate the autonomous loop.
            if action_id in (ACTION_TO_ID["NOOP"], ACTION_TO_ID["COMMIT"]):
                break

        return {"outputs": outputs, "states": states, "final_state": current}
