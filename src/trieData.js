...
let DataWrapper = require("./dataWrapper.js");

module.exports = class TrieData {
  ...
  async search (key) {
	...
	let result = (await (new HybridModel({
			...
			stack: stack,
			result: 4,
			symbolicWrapper: self.dataWrapper,
			symbolicFunctions: {
							...
							return (await (new HybridModel({
								...
								symbolicFunctions: {
									...
									getNextNodeId: async function(stack, args) {
										...
										var result;
										try {
											result = await self.dataWrapper.Get([`TrieModel`, stack[args[0]], stack[args[1]]]);
											// The getExchaustiveCheck will never return a negative result because there is no training.
											// Therefore, there's no reason to worry about accounting for height in getExchaustiveCheck.
											height++;
										}
										catch (error) {
											result = null;
									...
									getExchaustiveCheck: async function(stack, args) {
										...
										var result;
										try {
											result = await self.dataWrapper.Get([`TrieCheck`, stack[args[0]]]);
										}
										catch (error) {
											result = null;
										}
										
										return result != stack[args[1]];
									},
				...
				getNodeEndCheck: async function(stack, args) {
					...
					return await self.dataWrapper.Get([`TrieEnd`, stack[args[0]]]) == "FALSE";
  ...
  async insert (key) {
	...
	await (new HybridModel({
			...
			symbolicFunctions: {
				...
				getInsertLoop: async function(stack, args) {
					// genericSymbolicModel("getSearchLoop", stack, 3, []);
					...
					await (new LoopHybridModel({
						...
						loopCallback: async function() {							
							return (await (new HybridModel({
								model: {
									sequence: "InsertLoopSequence",
									method:  "InsertLoopMethod",
									argument:  "InsertLoopArg",
									result: "InsertLoopResult",
								},
								stack: stack,
								result: 3,
								symbolicWrapper: self.dataWrapper,
								symbolicFunctions: {
									...
									getNextNodeId: async function(stack, args) {
										...
										try {
											...
											await self.dataWrapper.Set([`TrieModel`, stack[args[0]], stack[args[1]], self.nodeCounter]);
											await self.dataWrapper.Set([`TrieEnd`, self.nodeCounter, false]);
											await self.dataWrapper.Set([`TrieCheck`, self.nodeCounter, stack[args[1]]]);
											result = self.nodeCounter++;
										}
				...
				setNodeEndWord: async function(stack, args) {
					// Set the EndWord model for the end of the word'
					await self.dataWrapper.Set([`TrieEnd`, stack[args[0]], true]);
					return false;
