...
| | Claim | Result |
| --- | --- | --- |
| H1 | Coverage ordering allocates fewer nodes than an arbitrary order | holds — 6.46%–11.83% fewer, across four relation slices |
| H2 | Branch-local ordering beats one global order | holds, narrowly — 1.51%–3.62%, for 20–40x the compute |
| H3 | Access depth is set by the individual, not the vocabulary size | holds — vocabulary 10x, nodes 7.9x, mean depth 0.9985x |
| H4 | The greedy heuristic lands near the true optimum | holds — exact optimum on 199 of 200 exhaustive samples |

Ordering alone decides 62,685 nodes between the best and worst arrangement of
the same facts. Details and caveats in [research/v683/README.md](research/v683/README.md);
generated output in `results/v683/`.
